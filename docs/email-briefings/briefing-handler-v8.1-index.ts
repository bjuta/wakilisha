...
// v8.1: Catalog-driven template routing (profiles in DB) + app-matching card styles (Inter, 14px radius, image-first artists, chart rank colors)
...
// Module→section type routing map
const MDS: Record<string, string[]> = {
  ...
  save_routes: ["articles", "guides"], memory_lead: ["articles"],
  language_cards: ["articles"], archive_threads: ["articles", "guides"],
  diaspora_lead: ["articles"], distance_cards: ["articles"], memory_archive: ["guides"],
  discovery_routes: ["articles", "releases"], admin_routes: ["articles", "guides"],
  ...
  spotlight_card: ["artists"], first_signal_tiles: ["artists"], cross_border_artists: ["artists"],
  ...
  regional_lead: ["articles"], city_board: ["articles", "artists"],
  regional_archive: ["guides", "articles"], agenda_hero: ["articles"],
  day_cards: ["articles", "releases"],
  label_cards: ["labels"], roster_motion: ["artists"], industry_routes: ["articles", "guides"],
  registry_stats: ["articles"], entity_change_cards: ["articles"], repair_notes: ["articles"],
};

// Helper: derive display label from module name
function ml(n: string): string {
  return n.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase());
}

// Helper: derive link URL from section type
...
// Helper: derive link label from section type
...
// Special modules with custom hero cards
const SPECIAL = new Set(["featured_routes", "lead_editorial", "chart_pulse", "chart_lead", "guide_hero"]);

// Modules with tinted background
const BG = new Set(["archive_routes", "archive_chart_route", "memory_archive", "regional_archive", "movement_board", "ranked_artwork_tiles", "guide_hero"]);
...
// ═══ APP-MATCHING STYLES (Inter, 14px radius, image-first artists) ═══
...
${bio ? `<div style="font-family:${FNT};font-size:11px;line-height:1.4;color:rgba(255,255,255,.70);max-height:2.8em;overflow:hidden;font-weight:600;">${s(bio)}</div>` : ""}
</div>
</div>
</td></tr>
</table>
...
function renderModule(
  ...
  function sectionHeader(): string {
    const linkTD = linkUrl && linkLabel
  ...
  // == Featured Story (featured_routes / lead_editorial) ==
...
<a class="wk-button" href="${s(fsUrl)}" style="display:inline-flex;align-items:center;gap:8px;border:1px solid ${s(accent)}44;border-radius:6px;padding:11px 18px;font-family:${FNT};font-size:13px;line-height:1;font-weight:700;letter-spacing:-.005em;color:${s(accent)};background:#fff;text-decoration:none;">Read the dispatch</a>
  ...
  // == Chart Pulse (chart_pulse / chart_lead) ==
  if (moduleName === "chart_pulse" || moduleName === "chart_lead") {
    ...
    const chartUrl = lead.edition_slug ? `${origin}/charts/${s(lead.edition_slug)}` : `${origin}/charts`;
...
<p class="wk-muted" style="margin:0;font-family:${FNT};font-size:13px;line-height:1.5;color:#6B6E62;">${s(chartTitle)} ${movementBadge(lead.movement)}</p>
    ...
    if (tiles.length > 0) {
      parts += `<div style="height:20px"></div><table role="presentation" width="100%">${genChartTrackTileRows(tiles, origin, accent)}</table>`;
    }
    parts += `</td></tr>`;
    return parts;
  }
...
// ═══ MAIN EMAIL GENERATOR (catalog-driven, profiles from DB) ═══
  ...
  for (const sub of subscribers) {
    ...
    try {
      if (RAK) {
        ...
        const resendHeaders: Record<string, string> = { "List-Unsubscribe": `<${mailtoUnsub}>, <${unsubUrl}>`, "List-Unsubscribe-Post": "List-Unsubscribe=One-Click" };
