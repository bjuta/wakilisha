-- Phase 5B Migration 219:
-- Preserve Registry Apple Music song identity in the cached Playlist
-- publication read model even when preview audio is the primary engine.

begin;

do $phase_5b_m219_preflight$
begin
  if to_regprocedure(
  ...
  then
    raise exception
      'STOP: M218 Playlist publication materializer is missing';
  end if;
  ...
  then
    raise exception
      'STOP: Registry provider-link authority is missing';
  end if;
  ...
  if not exists (
    ...
    raise exception
      'STOP: Registry track metadata compatibility source is missing';
  end if;
end;
$phase_5b_m219_preflight$;
