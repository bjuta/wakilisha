...
create table if not exists public.signal_os_entity_daily_metrics (
  ...
  share_copy_events integer not null default 0,
  share_click_events integer not null default 0,
  newsletter_events integer not null default 0,
  playback_events integer not null default 0,
  video_events integer not null default 0,
  scroll_events integer not null default 0,
  search_mentions integer not null default 0,
  referrer_domains integer not null default 0,
  first_seen_at timestamptz,
  last_seen_at timestamptz,
  sample_context jsonb not null default '{}'::jsonb,
  updated_at timestamptz not null default now(),
  primary key (metric_date, entity_type, entity_slug)
);

create table if not exists public.signal_os_entity_signal_scores (
  ...
  signal_label text not null default 'Quiet',
  page_views integer not null default 0,
  unique_sessions integer not null default 0,
  share_events integer not null default 0,
  search_mentions integer not null default 0,
  ...
  updated_at timestamptz not null default now(),
  primary key (score_date, entity_type, entity_slug)
);

create table if not exists public.signal_os_journey_edges_daily (
  metric_date date not null,
  ...
  updated_at timestamptz not null default now(),
  primary key (metric_date, from_type, from_slug, to_type, to_slug)
);

create table if not exists public.signal_os_search_demand_gaps (
  metric_date date not null,
  query text not null,
  searches integer not null default 0,
  unique_sessions integer not null default 0,
  zero_result_events integer not null default 0,
...
create table if not exists public.signal_os_content_opportunities (
...
create index if not exists signal_os_entity_daily_metrics_entity_idx
  on public.signal_os_entity_daily_metrics (entity_type, entity_slug, metric_date desc);

create index if not exists signal_os_entity_signal_scores_score_idx
  on public.signal_os_entity_signal_scores (score_date desc, signal_score desc);

create index if not exists signal_os_journey_edges_daily_from_idx
  on public.signal_os_journey_edges_daily (from_type, from_slug, metric_date desc);

create index if not exists signal_os_journey_edges_daily_to_idx
  on public.signal_os_journey_edges_daily (to_type, to_slug, metric_date desc);

create index if not exists signal_os_search_demand_gaps_score_idx
  on public.signal_os_search_demand_gaps (metric_date desc, opportunity_score desc);

create index if not exists signal_os_content_opportunities_status_idx
  on public.signal_os_content_opportunities (status, opportunity_date desc, opportunity_score desc);
