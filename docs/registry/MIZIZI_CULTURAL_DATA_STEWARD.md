...
Different ISRC values are retained as evidence but never auto-prove distinct
...
- PR #1034 originally closed same-Artist, same-slug public Track ambiguity by
  putting Registry Track UUID into the public Track route.
- that Production frontend deployment completed from exact merged main
  `ee52a276f61ff93aff948d2a2ff2be796c3e5312`;
- `public-content-read` Production authority at that closure was v92;
- at that point Chart scoring still carried continuity through
  `normalized_key` before canonical Track UUID authority was fully applied.
  That statement is historical evidence of the #1034 closure state, not a
  current routing requirement.
