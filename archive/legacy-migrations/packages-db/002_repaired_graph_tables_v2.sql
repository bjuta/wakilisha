...
-- 002_repaired_graph_tables_v2.sql
--
-- Corrected runnable version of the repaired graph schema.
-- Use this file instead of 002_repaired_graph_tables.sql.
...
create table if not exists wakilisha_repaired.entity_slugs (
  ...
  constraint entity_slugs_status_check_v2 check (status in ('active', 'redirect', 'retired', 'duplicate', 'review'))
);
...
create unique index if not exists track_artists_dedupe_idx
  on wakilisha_repaired.track_artists(
    track_id,
    artist_id,
    coalesce(role, ''),
    coalesce(position, -1)
...
create unique index if not exists release_tracks_dedupe_idx
  on wakilisha_repaired.release_tracks(
    release_id,
    track_id,
    coalesce(disc_number, 1),
    coalesce(track_number, -1)
  );
...
create unique index if not exists artist_genres_dedupe_idx
  on wakilisha_repaired.artist_genres(artist_id, genre_id);
...
create table if not exists wakilisha_repaired.content_route_classification (
  ...
  source_payload jsonb,
  created_at timestamptz not null default now(),
  constraint content_route_classification_check_v2 check (
...
create table if not exists wakilisha_repaired.review_queue (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint review_queue_status_check_v2 check (status in ('open', 'accepted', 'rejected', 'resolved', 'ignored'))
);
