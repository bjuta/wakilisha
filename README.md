...
This repository is being prepared as the clean React-era home for WAKILISHA.

The first milestone is **not UI**. The first milestone is **data repair**.
...
## Structured Supabase import

The raw CSV export has been reorganized into a developer-friendly bundle:

```text
wakilisha_supabase_import_2026-05-30.zip
```

Unzip it locally into:
...
The raw CSVs are intentionally ignored by Git. They are large source data files, not application source code. Keep them locally under the `raw/` folder and load them through repeatable migration scripts.
