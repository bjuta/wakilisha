...
-- - 73 current public "Wakilisha Staff" Articles receive institutional Author Credit.
-- - visible/historical bylines remain "Wakilisha Staff".
-- - Person authority and the 134 accepted human Article Credits remain untouched.
...
declare
  ...
  v_snapshot_match_count bigint;
  v_manifest_digest text;
  v_existing_attachment_count bigint;
  v_org_id uuid;
  v_credit_id uuid;
begin
  if not exists (
    ...
    raise exception
      'STOP: pgcrypto is required for the locked Staff manifest digest';
  end if;

  create temporary table pg_temp.organization_staff_manifest
  on commit drop
  as
  select
    ...
    article.slug,
    btrim(coalesce(article.author, '')) as legacy_author,
    btrim(coalesce(version.author_display, '')) as version_author_display
  ...
  into
    v_staff_count,
    v_snapshot_match_count,
    v_manifest_digest
  from pg_temp.organization_staff_manifest;

  if v_staff_count <> 73
     or v_snapshot_match_count <> 73
  then
    raise exception
      'STOP: reviewed Staff Article boundary moved: current %, matching version snapshots %',
      v_staff_count,
      v_snapshot_match_count;
  end if;

  if v_manifest_digest <>
     'eda3b2b8708a10416004bc12bdef28c42a0944e9a5f848305aa8c7b7c78f7067'
  then
    raise exception
      'STOP: reviewed Staff Article manifest digest moved';
  end if;

  select count(*)
  into v_existing_attachment_count
  from pg_temp.organization_staff_manifest manifest
  ...
  if v_existing_attachment_count <> 0 then
    raise exception
      'STOP: Staff Article Credit attachment boundary moved: expected 0, found %',
      v_existing_attachment_count;
  end if;
  ...
  if v_org_id is null then
    raise exception
      'STOP: WAKILISHA Organization route is missing';
  end if;
  ...
  if v_credit_id is null then
    ...
    values (
      ...
      'WAKILISHA',
      'Author',
      'Canonical institutional Article author reconciliation',
      null,
      null,
      null
    )
