...
as $$
  ...
  actionable_artists as (
    ...
    where r.artist_id is null
       or nullif(r.origin_iso2, '') is null
  ),
  grouped as (
    select
      case
    ...
    from actionable_artists
    where issue_type in ('unresolved_artist', 'missing_origin')
    group by
      review_key,
      issue_type,
      token_slug,
...
begin
  ...
  values (
    ...
    v_origin,
    'create_artist_shell',
    'Created or updated from chart origin queue.',
    p_actor_user_id
  );
...
grant execute on function public.chart_reset_run_after_origin_resolution(text) to service_role;
