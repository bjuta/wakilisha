-- Phase 7B V4C: governed public Video quality selection.
--
-- Extends the accepted V4B adaptive_delivery object with governed
-- rendition playlist metadata for manual quality selection.
-- MP4 fallback, the HLS master, and V4A processing authority are unchanged.
...
begin
  ...
  then
    raise exception
      'STOP: accepted V4B public Video reader is missing';
  end if;
  ...
  if position('adaptive_delivery' in v_reader) = 0
     or position('rendition_count' in v_reader) = 0
  ...
  then
    raise exception
      'STOP: public Video reader is not at the accepted V4B baseline';
  end if;
end;
$phase_7b_v4c_preflight$;
