...
function sanitizeVcShortcodes(html: string): string {
  ...
  result = result.replace(/\[\/uncode_[^\]]*?\]/gi, "");
  result = result.replace(/\[caption[^\]]*?\]([\s\S]*?)\[\/caption\]/gi, (_match: string, inner: string) => {
...
function extractTerms(embedded: Record<string, unknown> | undefined): Array<{ taxonomy: string; slug: string; name: string }> {
  ...
  for (const group of termGroups) {
    for (const term of group) {
      terms.push({ taxonomy: String(term.taxonomy ?? ""), slug: String(term.slug ?? ""), name: String(term.name ?? "") });
    }
  }
  return terms;
}
...
function getTitle(item: Record<string, unknown>): string {
  if (typeof item.title === "object" && item.title) return (item.title as Record<string, string>).rendered || "";
  return String(item.id ?? "");
}
function getContent(item: Record<string, unknown>): string {
  if (typeof item.content === "object" && item.content) return (item.content as Record<string, string>).rendered || "";
  return "";
}
function getExcerpt(item: Record<string, unknown>): string {
  if (typeof item.excerpt === "object" && item.excerpt) return (item.excerpt as Record<string, string>).rendered || "";
  return "";
}
function buildStageRecord(runId: string, item: Record<string, unknown>, postType: string, sourceFile: string): Record<string, unknown> {
  ...
  if (entry && targetStatus !== "ready") warnings.push(`WAKILISHA CPT ${postType} mapped to ${targetEntity}; review metadata/relationships before finalization.`);
  if (wpStatus !== "publish") warnings.push(`Post status "${wpStatus}" preserved — will remain draft on promotion.`);
  const vcCleaned = cleanContent !== rawContent;
  const rawRecord = {
    wp_id: wpId, wp_type: postType, wp_link: String(item.link ?? ""), wp_status: wpStatus,
    date, modified, title: stripHtml(title), content: cleanContent, content_import_cleaned: vcCleaned,
    excerpt: rawExcerpt, slug, featured_image: featuredImage, terms, meta: wpMeta,
    post_author: authorId || null, author_name: authorName || null,
  };
  const mappedRecord = {
    title: stripHtml(title), body: cleanContent, excerpt: cleanExcerpt, slug,
    source_status: wpStatus, post_type: postType, canonical_kind: entry?.canonical_kind ?? postType,
    wakilisha_cpt: Boolean(entry), published_at: parseDate(date), modified_at: parseDate(modified),
    featured_image_url: featuredImage?.url || null, featured_image_alt: featuredImage?.alt || null,
    terms_count: terms.length, wp_custom_fields_count: Object.keys(wpMeta).length,
    vc_shortcodes_cleaned: vcCleaned, post_author: authorId || null, author_name: authorName || null,
  };
  return {
    ingestion_run_id: runId, source_kind: "wordpress_rest_api", source_file: sourceFile,
    source_entity: `wp_api.${postType}`, source_record_id: wpId || null, source_slug: postName || null,
    target_entity: targetEntity, target_status: targetStatus, target_slug: slug || null,
    title: stripHtml(title) || null, body: cleanContent || null, excerpt: cleanExcerpt || null,
    published_at: parseDate(date), author_name: authorName || null, source_url: String(item.link ?? "") || null,
    raw_record: rawRecord, mapped_record: mappedRecord,
    mapping_candidate_ids: entry ? [`wakilisha-cpt-${postType}`] : [postType === "post" ? "wp-post" : postType === "page" ? "wp-page" : `wp-${postType}`],
    warnings, errors: targetStatus === "blocked" ? ["Cannot stage as ready without a title or recognized post type."] : [],
  };
}
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !supabaseKey) {
    return new Response(JSON.stringify({ error: "Supabase service role key missing. This function requires SERVICE_ROLE_KEY to write staging records." }), {
  ...
  try {
    ...
    if (!runId) return new Response(JSON.stringify({ error: "runId is required." }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const { data: run, error: runErr } = await supabase.from("wk_ingestion_runs").select("*").eq("id", runId).maybeSingle();
    if (runErr || !run) return new Response(JSON.stringify({ error: "Ingestion run not found." }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    if (!siteUrl) return new Response(JSON.stringify({ error: "No site_url in source_manifest." }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    await supabase.from("wk_ingestion_runs").update({ status: "staging", started_at: new Date().toISOString(), errors: [] }).eq("id", runId);
    ...
    const typeDiags: Record<string, { expectedTotal: number; fetchedCount: number; stagedCount: number; draftCount: number; pagesFetched: number; apiOk: boolean; errorMessage?: string; isAggregateCpt: boolean; warning?: string; }> = {};

    let typesToFetch: string[] = [];
    if (buckets.length > 0) {
      for (const bucket of buckets) { const sourceFiles = bucket.source_files as string[] | undefined; if (sourceFiles && sourceFiles.length > 0) typesToFetch.push(...sourceFiles); }
      ...
      if (postTypes) typesToFetch = Object.keys(postTypes).filter((k) => k !== "__error");
    }
    if (typesToFetch.length === 0) typesToFetch = ["posts", "pages"];
    ...
    for (const postType of typesToFetch) {
      ...
      typeDiags[postType] = { expectedTotal: 0, fetchedCount: 0, stagedCount: 0, draftCount: 0, pagesFetched: 0, apiOk: false, isAggregateCpt: isAggregate };

      let totalItems = 0;
      try {
        const countRes = await fetch(`${siteUrl}/wp-json/wp/v2/${restBase}?per_page=1&_embed`, { headers: { "Accept": "application/json", "User-Agent": "Wakilisha/1.0" } });
        if (countRes.ok) { totalItems = parseInt(countRes.headers.get("X-WP-Total") || "0", 10); typeDiags[postType].expectedTotal = totalItems; typeDiags[postType].apiOk = true; }
        else typeDiags[postType].errorMessage = `HTTP ${countRes.status}`;
      } catch (err) { typeDiags[postType].errorMessage = err instanceof Error ? err.message : "Connection failed"; }

      if (totalItems === 0 && typeDiags[postType].errorMessage) { stats.skipped++; if (isAggregate) typeDiags[postType].warning = `This post type is known to store data in postmeta, not as individual CPT posts. REST API only sees ${totalItems} items. Use the MySQL direct-connect pipeline (scripts/imports/stage-wordpress-database-records.ts) to import its postmeta data.`; continue; }
      if (totalItems === 0) { stats.skipped++; continue; }

      if (isAggregate && totalItems <= 5) typeDiags[postType].warning = `REST API only exposes ${totalItems} items for this post type. However, the actual track/release/label data is stored in WordPress postmeta, not as individual CPT posts. The MySQL direct-connect pipeline (scripts/imports/stage-wordpress-database-records.ts) can import postmeta data — the REST API cannot.`;
      ...
      for (let page = 1; page <= pages; page++) {
        try {
          const wpRes = await fetch(`${siteUrl}/wp-json/wp/v2/${restBase}?per_page=${API_PAGE_SIZE}&page=${page}&orderby=date&order=desc&_embed`, { headers: { "Accept": "application/json", "User-Agent": "Wakilisha/1.0" } });
          if (!wpRes.ok) { allFailures.push({ ingestion_run_id: runId, source_file: `wp_api.${postType}`, source_entity: postType, failure_stage: "fetch", message: `HTTP ${wpRes.status} on page ${page}`, raw_record: { postType, page } }); stats.failed++; continue; }
          ...
          for (const item of items) {
            ...
            if (wasCleaned?.content_import_cleaned === true) stats.vcCleaned++;
            if (stageRecord.author_name && String(stageRecord.author_name).length > 0) stats.authorsFound++;
            if (wpStatus !== "publish") { stats.drafts++; draftCounts[targetEnt] = (draftCounts[targetEnt] || 0) + 1; typeDiags[postType].draftCount++; }
            if (stageRecord.target_status === "ready") { stats.staged++; typeDiags[postType].stagedCount++; }
          }
        } catch (pageErr) {
          const msg = pageErr instanceof Error ? pageErr.message : "network error";
          allFailures.push({ ingestion_run_id: runId, source_file: `wp_api.${postType}`, source_entity: postType, failure_stage: "fetch", message: msg, raw_record: { postType, page } });
    ...
    for (let i = 0; i < allRecords.length; i += BATCH_SIZE) {
      ...
      try { await insertBatch(supabase, batch); } catch (batchErr) { const msg = batchErr instanceof Error ? batchErr.message : "insert error"; errors.push(`Batch insert ${i}-${i + batch.length}: ${msg}`); }
    }
    ...
    for (const [pt, diag] of Object.entries(typeDiags)) { if (diag.warning) aggregateWarnings.push(`[${pt}] ${diag.warning}`); }

    const stagingSummary = {
      staged_at: new Date().toISOString(), processor: "process-wp-import-v6", version: "6.0.0",
      records: allRecords.length, failures: allFailures.length,
      counts_by_target_entity: entityCounts, draft_counts: draftCounts,
      vc_shortcodes_cleaned: stats.vcCleaned, authors_found: stats.authorsFound,
      wakilisha_cpt_map_enabled: true, production_import_enabled: false,
      type_diagnostics: typeDiags, aggregate_cpt_warnings: aggregateWarnings,
    };

    const updatedManifest = { ...(manifest as Record<string, unknown>), staging: stagingSummary };

    const vcMsg = stats.vcCleaned > 0 ? `${stats.vcCleaned} record(s) had VC/WPBakery shortcodes stripped at import time.` : "No VC/WPBakery shortcodes detected in imported content.";
    const authorMsg = stats.authorsFound > 0 ? `${stats.authorsFound} record(s) had post_author data captured from _embedded.author.` : "No author data found in _embedded responses.";
    const captionMsg = "v6: WP [caption] shortcodes now converted to <figure><figcaption> HTML for proper image caption display; no anon key fallback.";

    const updatedWarnings = Array.from(new Set([
      ...(run.warnings ?? []),
      "v6: Records staged via WordPress REST API with post_author extraction from _embedded.author and proper caption handling.",
      vcMsg, authorMsg, captionMsg,
    ...
    await supabase.from("wk_ingestion_runs").update({
      status: "staged", finished_at: new Date().toISOString(), imported_counts: entityCounts,
      source_manifest: updatedManifest, warnings: updatedWarnings, errors: errors.slice(0, 200),
    }).eq("id", runId);

    return new Response(JSON.stringify({ success: true, runId, stats: { total: stats.total, staged: allRecords.length, ready: stats.staged, drafts: stats.drafts, failed: allFailures.length, vcShortcodesCleaned: stats.vcCleaned, authorsFound: stats.authorsFound }, entityCounts, draftCounts, errorCount: errors.length, typeDiagnostics: typeDiags }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
