...
function cleanPublicMusicSlug(storedSlugRaw: unknown, titleRaw: unknown, artistSlugRaw: unknown): string {
  ...
  if (storedSlug === titleThenArtist || storedSlug.startsWith(`${titleThenArtist}-`)) return titleSlug;
  if (storedSlug === artistThenTitle || storedSlug.startsWith(`${artistThenTitle}-`)) return titleSlug;
...
async function findReleaseByScopedPublicSlug(
  ...
  const { data: links } = await supabase
    ...
    .select("release_id, artist_slug, is_primary, credit_order, status")
    .eq("artist_slug", artistSlug)
    .in("status", ["active"])
    .order("is_primary", { ascending: false })
    .order("credit_order", { ascending: true })
  ...
  const releaseIds = [...new Set((links ?? []).map((row: any) => String(row.release_id)).filter(Boolean))];
  ...
  const matches = (releases ?? []).filter((release: any) => {
    const registrySlug = slugify(String(release.slug || ""));
    const publicReleaseSlug = cleanPublicMusicSlug(release.slug, release.title, artistSlug);
    return registrySlug === publicSlug || publicReleaseSlug === publicSlug;
  });
  ...
  return matches.sort((a: any, b: any) => {
    ...
    const aDate = new Date(String(a.release_date || "1900-01-01")).getTime();
    const bDate = new Date(String(b.release_date || "1900-01-01")).getTime();
    return bDate - aDate;
  })[0] ?? null;
}
...
function buildEntryItem(e: any) {
  ...
  return {
    ...
    trackSlug: String(e.__publicTrackSlug || e.track_slug || ""),
    trackTitle: String(e.__publicTrackTitle || e.track_title || ""),
    canonicalTrackId: e.__publicTrackId ? String(e.__publicTrackId) : (e.canonical_track_id ? String(e.canonical_track_id) : null),
    artistNames: publicArtistNames,
    artistSlugs: publicArtistSlugs,
    artworkUrl: e.__publicArtworkUrl || e.artwork_url || null,
    score: e.total_score != null ? Number(e.total_score) : null,
    sourceEntryId: String(e.id),
  };
}

async function resolvePublicChartEntryIdentity(supabase: any, entries: any[]): Promise<any[]> {
  ...
  const { data: canonicalTrackRows } = allTrackIds.length > 0
    ? await supabase
        .from("registry_tracks")
  ...
  const publicTrackById = new Map(
    (canonicalTrackRows ?? []).map((track: any) => [
      String(track.id),
      {
        id: String(track.id),
        slug: String(track.slug || ""),
  ...
  function resolveArtist(sourceSlug: string, displayName: string): { slug: string; name: string } {
    ...
    return {
      ...
      name: resolved?.name || displayName || cleanSlug.replace(/-/g, " "),
    };
  }
  ...
  return sourceEntries.map((entry: any) => {
    ...
    const publicTrack =
      entryTrackId
        ? publicTrackById.get(entryTrackId)
        : null;
    const publicTrackFields = publicTrack
      ? {
          __publicTrackId: publicTrack.id,
          __publicTrackSlug: publicTrack.slug,
          __publicTrackTitle: publicTrack.title,
          __publicArtworkUrl: publicTrack.artworkUrl,
        }
      : {};
...
async function fetchActiveReleaseTrackCounts(
  supabase: ReturnType<typeof createClient>,
  releaseIdsRaw: string[],
  ...
  for (let offset = 0; offset < releaseIds.length; offset += 500) {
    const ids = releaseIds.slice(offset, offset + 500);
  ...
  try {
    ...
    if (path === "/health" || path === "/health/") {
      ...
      const [articleResult, artistResult, releaseResult, chartResult] = await Promise.all([
        ...
        supabase.from("wk_chart_entries_v2").select("rank, track_title, track_slug, artist_name, artist_slug, artwork_url, edition_id, canonical_track_id").order("rank", { ascending: true }).limit(20)
      ]);
      const authorPaths = await loadPublicArticleAuthorPathMap(supabase);
      const authorOrganizationPaths = await loadPublicArticleAuthorOrganizationPathMap(supabase);
      ...
      const releaseTrackCounts = await fetchActiveReleaseTrackCounts(
        supabase,
        releaseRows.map((release: any) => String(release.id || "")),
      );
      const releases = releaseRows
        .filter((release: any) => (releaseTrackCounts.get(String(release.id)) || 0) > 1)
        .map((r: any) => ({ contentType: "release" as const, id: String(r.id), slug: String(r.slug), title: String(r.title), section: String(r.release_type || "Release"), dek: r.description || "", author: "", authorSlug: "", date: r.release_date ? String(r.release_date).split("T")[0] : "", readingTime: 0, heroUrl: String(r.artwork_url || ""), tags: [], releaseType: String(r.release_type || "album") }));
      const chartRows = await resolvePublicChartEntryIdentity(
        supabase,
        chartResult.data ?? [],
      );
      const chartHighlights = chartRows.map((row: any) => {
        const item = buildEntryItem(row);
        const artistName = item.artistNames.join(", ");
        return { contentType: "chart_entry" as const, id: String(item.id || row.edition_id), slug: item.trackSlug, title: item.trackTitle, section: "Chart Entry", dek: `#${item.rank} \u00B7 ${artistName}`, author: artistName, authorSlug: item.artistSlugs[0] || "", date: "", readingTime: 0, heroUrl: String(item.artworkUrl || ""), tags: [], rank: item.rank, artistName };
      });
      ...
      const { data: chartEntriesBySlug } = await supabase.from("wk_chart_entries_v2").select("rank, track_title, track_slug, movement, previous_rank, artwork_url, edition_id, artist_name, artist_slug, canonical_track_id").eq("artist_slug", slug).order("rank", { ascending: true }).limit(50);
      let chartEntries = chartEntriesBySlug ?? [];
      if (chartEntries.length === 0 && displayName) { const { data: chartEntriesByName } = await supabase.from("wk_chart_entries_v2").select("rank, track_title, track_slug, movement, previous_rank, artwork_url, edition_id, artist_name, artist_slug, canonical_track_id").ilike("artist_name", displayName).order("rank", { ascending: true }).limit(50); chartEntries = chartEntriesByName ?? []; }
      chartEntries = await resolvePublicChartEntryIdentity(
        supabase,
        chartEntries,
      );
      const chartEntryList = chartEntries.map((e: any) => {
        const item = buildEntryItem(e);
        ...
        if (prev > 0 && curr > 0) {
          if (curr < prev) {
            movement = "up";
            movementAmount = prev - curr;
          } else if (curr > prev) {
            movement = "down";
            movementAmount = curr - prev;
          }
        }
        return {
          rank: curr,
      ...
      if (!artists || artists.length === 0) {
        ...
        const publicReleaseTrackCounts =
          await fetchActiveReleaseTrackCounts(
            supabase,
            releaseIds,
          );
        ...
        for (const row of releaseArtistRows) {
          const releaseId = String(row.release_id || "");
          if ((publicReleaseTrackCounts.get(releaseId) || 0) <= 1) {
            continue;
          }
          const artistId = String(row.artist_id || "");
          const artistSlug = String(row.artist_slug || "");
      ...
      data = { release: { id: releaseId, slug: cleanPublicMusicSlug(release.slug, release.title, primaryArtistSlug || urlArtistSlug || ""), title: String(release.title), artist: artistName, year: release.release_date ? String(release.release_date).split("-")[0] : "", releaseDate: release.release_date || "", releaseType: String(release.release_type || "album"), labelName, labelSlug: label?.slug || "", artworkUrl: release.artwork_url || "", trackCount: trackList.length, tracks: trackList, totalDuration, description, featuredArtists: allFeaturedArtists, chartStats: releaseChartStats, metadata: { ...releaseMeta, wpLabel: releaseMeta.wp_label || null, wpDistributor: releaseMeta.wp_distributor || null, wpWriters: releaseMeta.wp_writers || null, wpProducers: releaseMeta.wp_producers || null } } };
    }
      ...
      const { data: memberships } = releaseIds.length > 0
        ? await supabase
            .from("registry_release_tracks")
            .select("release_id, track_id")
      ...
      const { data: releaseArtists } = publicReleaseIds.length > 0
        ? await supabase
            .from("registry_release_artists")
            .select("release_id, artist_name_text, artist_slug, is_primary, credit_order")
            .in("release_id", publicReleaseIds)
            .eq("status", "active")
            .order("is_primary", { ascending: false })
            .order("credit_order", { ascending: true })
        : { data: [] };

      const artistByRelease = new Map<string, string>();
      for (const credit of releaseArtists ?? []) {
        const releaseId = String(credit.release_id || "");
        if (!releaseId || artistByRelease.has(releaseId)) continue;
        artistByRelease.set(
          releaseId,
          String(
      ...
      const labelIds = publicReleases
        .map((release: any) => release.label_id)
        .filter(Boolean)
        .map(String);
      ...
      data = {
        releases: publicReleases.map((release: any) => ({
          id: String(release.id),
          slug: String(release.slug),
          title: String(release.title),
          artist:
            artistByRelease.get(String(release.id)) ||
            "Unknown",
          ...
          labelName:
            labelMap.get(String(release.label_id)) ||
            "Independent",
      ...
      const { data: label } = await supabase
        .from("registry_labels")
        .select("id, slug, name, description, country_code, status")
        .eq("slug", lSlug)
        .eq("status", "active")
        .maybeSingle();

      if (!label) {
        return jsonResponse({ data: null }, origin, 404);
      }
      ...
      const [byLabelId, byMeta] = await Promise.all([
        ...
        supabase
          .from("registry_releases")
          .select("id, slug, title, release_date, release_type, artwork_url, label_id, metadata")
          .filter("metadata->>record_label", "eq", labelName)
          .in("status", ["active", "draft"])
          .order("release_date", { ascending: false })
          .limit(500),
      ]);
      ...
      for (const row of [
        ...
        seen.add(id);
        allLabelReleases.push(row);
      }

      const releaseIds = allLabelReleases.map(
        (release: any) => String(release.id),
      );
      const { data: releaseMemberships } = releaseIds.length > 0
        ? await supabase
            .from("registry_release_tracks")
            .select("release_id, track_id")
            .in("release_id", releaseIds)
      ...
      for (const membership of releaseMemberships ?? []) {
        const releaseId = String(membership.release_id || "");
        if (!releaseId) continue;
        releaseTrackCount.set(
          releaseId,
          (releaseTrackCount.get(releaseId) || 0) + 1,
        );
      }

      const releases = allLabelReleases.filter(
        (release: any) =>
          (releaseTrackCount.get(String(release.id)) || 0) > 1,
      );
      const publicReleaseIds = releases.map(
        (release: any) => String(release.id),
      );

      const artistForRelease = new Map<string, string>();
      if (publicReleaseIds.length > 0) {
        ...
        for (const credit of relArtists ?? []) {
          const releaseId = String(credit.release_id || "");
          if (!releaseId || artistForRelease.has(releaseId)) continue;
          artistForRelease.set(
            releaseId,
            String(
              credit.artist_name_text ||
              credit.artist_slug ||
      ...
      const rosterMap = new Map<
        string,
        { slug: string; name: string; artworkUrl: string }
      >();
      if (releaseIds.length > 0) {
        const { data: allReleaseArtists } = await supabase
          .from("registry_release_artists")
          .select("artist_slug, artist_name_text, artist_id, release_id")
          .in("release_id", releaseIds)
          .eq("status", "active");

        for (const credit of allReleaseArtists ?? []) {
          const artistSlug = String(credit.artist_slug || "");
          if (!artistSlug || rosterMap.has(artistSlug)) continue;
          rosterMap.set(artistSlug, {
            slug: artistSlug,
            name: String(
              credit.artist_name_text ||
              artistSlug,
      ...
      if (rosterSlugs.length > 0) {
        ...
        for (const artist of rosterArtists ?? []) {
          const entry = rosterMap.get(String(artist.slug));
          if (entry) {
            entry.artworkUrl =
      ...
      data = {
        label: {
          ...
          countryCode: label.country_code || null,
        },
        ...
        releases: releases.map((release: any) => ({
          ...
          artworkUrl: release.artwork_url || "",
          trackCount:
            releaseTrackCount.get(String(release.id)) || 0,
          artistName:
            artistForRelease.get(String(release.id)) || "",
        })),
        relatedLabels: (relatedLabels ?? []).map((related: any) => ({
      ...
      const { data: labels } = await supabase
        .from("registry_labels")
        .select("id, slug, name, normalized_name, country_code, description, status")
        .eq("status", "active")
      ...
      const publicReleaseRows = (releaseRows ?? []).filter(
        (release: any) =>
          (releaseTrackCounts.get(String(release.id)) || 0) > 1,
      );
      const publicReleaseIds = publicReleaseRows.map(
        (release: any) => String(release.id),
      );

      const { data: releaseArtistRows } = publicReleaseIds.length > 0
        ? await supabase
            .from("registry_release_artists")
      ...
      const artistsByRelease = new Map<string, Array<{ slug: string; name: string }>>();
      for (const credit of releaseArtistRows ?? []) {
        const releaseId = String(credit.release_id || "");
        if (!releaseId) continue;
        const rows = artistsByRelease.get(releaseId) || [];
        const slug = String(credit.artist_slug || "");
        const name = String(credit.artist_name_text || slug);
        if (slug || name) rows.push({ slug, name });
        artistsByRelease.set(releaseId, rows);
      }
        ...
        labels: (labels ?? []).map((label: any) => {
          ...
          const labelKeys = new Set(
            [
              ...
              String(label.normalized_name || "").trim().toLowerCase(),
            ].filter(Boolean),
          );
          const matchingReleases = publicReleaseRows.filter((release: any) => {
            if (String(release.label_id || "") === labelId) return true;
            ...
            return Boolean(rawLabel && labelKeys.has(rawLabel));
          });
          const artistKeys = new Set<string>();
          const featuredArtists: string[] = [];

          for (const release of matchingReleases) {
            for (const artist of artistsByRelease.get(String(release.id)) || []) {
          ...
          return {
            ...
            slug: String(label.slug),
            name: String(label.name),
            country: label.country_code || null,
            logoUrl: null,
      ...
      if (release) {
        ...
        if (relTrackRows && relTrackRows.length > 0) {
          ...
          releaseTracks = relTrackRows.map((rt: any) => {
            ...
            return { id: String(t.id), slug: cleanPublicMusicSlug(t.slug || t.id, t.title || "", primaryArtist?.slug || fallbackArtistSlug || ""), title: String(t.title || ""), artist: artistStr, duration: durationSeconds, trackNumber: Number(rt.track_number || 0), artworkUrl: t.artwork_url || "", previewUrl: t.preview_url || undefined };
          }).filter(Boolean);
        }
      }
      ...
      async function loadLatestPayload(cslug: string, marketSlug?: string | null) {
        ...
        return {
          ...
          entries: (await resolvePublicChartEntryIdentity(supabase, entries ?? [])).map(buildEntryItem),
        };
      }
      ...
      async function loadEntriesPayload(cslug: string, editionSlug: string, marketSlug?: string | null) {
        ...
        return { entries: (await resolvePublicChartEntryIdentity(supabase, entries ?? [])).map(buildEntryItem) };
      }
