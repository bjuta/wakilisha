// Phase 6B M2 public Audio transport adapter.
//
// This function owns no Audio authority. It uses the same anonymous RPC
// boundary as the public product and only presents that governed projection as
// RSS XML or a stable enclosure redirect.
import { createClient } from "https://esm.sh/@supabase/supabase-js@2";
import { renderAudioShowRss } from "./rss.ts";
...
function badRequest(method: string): Response {
  return response(
    method === "HEAD" ? null : "Invalid Audio delivery request\n",
    400,
    {
      "Content-Type": "text/plain; charset=utf-8",
...
Deno.serve(async (request) => {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    return response(
      method === "HEAD" ? null : "Audio delivery unavailable\n",
      503,
      {
        "Content-Type": "text/plain; charset=utf-8",
        "Cache-Control": "no-store",
