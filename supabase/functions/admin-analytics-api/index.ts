...
function scanResultToBrokenEvent(result: any) {
  ...
  const suggestedFix = brokenSuggestedFix(path);
  ...
  return {
    ...
    context: {
      status_code: status,
      not_found_path: path,
      route_guess: result.soft404 ? "suspected_soft_404" : routeGuess,
...
async function taxonomyEmptyEvents(db: ReturnType<typeof createClient>, pageRows: any[]) {
  ...
  for (const row of pageRows) {
    ...
    const table = kind === "tag" ? "wk_tags" : "wk_categories";
    const relationTable = kind === "tag" ? "wk_article_tags" : "wk_article_categories";
    const relationColumn = kind === "tag" ? "tag_id" : "category_id";
...
async function scanBrokenPages(db: ReturnType<typeof createClient>, range: RangeInput, limit = 80) {
  ...
  for (const url of candidates) {
    ...
    if (!result.ok) {
      brokenEvents.push(scanResultToBrokenEvent(result));
    } else {
      skipped += 1;
    }
  }
