...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── MAGAZINE PUBLIC ISSUE ──
      ...
      data = {
        issue: { id: String(issue.id), slug: String(issue.slug), title: String(issue.title), dek: issue.dek || null, status: String(issue.status), timeframeStart: issue.timeframe_start || null, timeframeEnd: issue.timeframe_end || null, issueType: String(issue.issue_type), visualFamily: issue.visual_family || null, treatment: issue.treatment || null, palette: issue.palette || null, contrastMode: issue.contrast_mode || null, createdBy: String(issue.created_by), publishedAt: issue.published_at || null },
        sections: (sections ?? []).map((s: any) => ({ id: String(s.id), spreadId: String(s.spread_id), sectionType: String(s.section_type), title: String(s.title), deck: s.deck || null, body: s.body || null, layout: String(s.layout), sortOrder: Number(s.sort_order), status: String(s.status), visualAssetId: s.visual_asset_id || null })),
        entities: (entities ?? []).map((e: any) => ({ id: String(e.id), sectionId: e.section_id || null, entityType: String(e.entity_type), entityId: String(e.entity_id), role: String(e.role), selectionState: String(e.selection_state), sortOrder: Number(e.sort_order), sourceReason: e.source_reason || null })),
    ...
    // ── ARTIST DETAIL ──
      ...
      if (Array.isArray(youtubeVideos)) { for (const videoUrl of youtubeVideos as string[]) { const v = String(videoUrl || ""); const ytMatch = v.match(/youtube\.com\/watch\?v=([^&]+)/) || v.match(/youtu\.be\/([^?&]+)/); if (ytMatch) { const videoId = ytMatch[1]; videos.push({ id: videoId, title: "Video", url: "https://www.youtube.com/embed/" + videoId + "?rel=0&modestbranding=1", thumbnail: "https://img.youtube.com/vi/" + videoId + "/mqdefault.jpg", platform: "youtube" }); } } }
      if (videos.length === 0) {
        const videoUrlsRaw = meta.video_urls;
        if (typeof videoUrlsRaw === "string" && videoUrlsRaw) { try { const parsed = JSON.parse(videoUrlsRaw); const urls = Array.isArray(parsed) ? parsed : []; for (const vid of urls) { const videoUrl = String(vid.url || vid || ""); const ytMatch = videoUrl.match(/youtube\.com\/watch\?v=([^&]+)/) || videoUrl.match(/youtu\.be\/([^?&]+)/); if (ytMatch) { const videoId = ytMatch[1]; videos.push({ id: videoId, title: String(vid.title || "Video"), url: "https://www.youtube.com/embed/" + videoId + "?rel=0&modestbranding=1", thumbnail: "https://img.youtube.com/vi/" + videoId + "/mqdefault.jpg", platform: "youtube" }); } } } catch { /* ignore */ } }
      }
    ...
    // ── ARTISTS LISTING ──
      ...
      data = { artists: (artists ?? []).map((a: any) => { const meta = (a.metadata || {}) as Record<string, unknown>; const genresArr = meta.genres; const artistGenres: string[] = []; if (Array.isArray(genresArr)) { for (const g of genresArr as string[]) artistGenres.push(String(g)); } return { id: String(a.id), slug: String(a.slug), name: String(a.display_name), country: a.origin_iso2 || null, imageUrl: a.public_image_url || null, genres: artistGenres, trackCount: 0, releaseCount: 0, isChartArtist: true, isRising: false, topChartPosition: null }; }) };
    }
    ...
    // ── RELEASE DETAIL ──
      ...
      const trackList = (tracks ?? []).map((t: any) => ({ id: String(t.id), slug: String(t.slug || t.id), title: String(t.title), artist: "Unknown", duration: Number(t.duration_ms || 0) / 1000, trackNumber: t.track_number || 0, artworkUrl: t.artwork_url || "" }));
    ...
    // ── LABEL DETAIL ──
      ...
      if (uniqueTrackSlugs.length > 0) {
        ...
        for (const c of (chartData ?? [])) { const artistSlug = String(c.artist_slug || ""); if (artistSlug && !rosterMap.has(artistSlug)) rosterMap.set(artistSlug, { slug: artistSlug, name: String(c.artist_name || artistSlug), artworkUrl: c.artwork_url || "" }); }
      }
    ...
    // ── CHARTS — list all programs ──
      ...
      const programsWithEditions = await Promise.all((programs ?? []).map(async (p: any) => {
        ...
        const latestEdition = editions && editions.length > 0
          ? { id: String(editions[0].edition_slug), slug: String(editions[0].edition_slug), label: String(editions[0].edition_label), date: String(editions[0].edition_date), periodStart: editions[0].period_start || null, periodEnd: editions[0].period_end || null, entryCount: editions[0].entry_count || 0 }
          : null;
        ...
        return {
          ...
          archive: (editions ?? []).map((e: any) => ({
            id: String(e.edition_slug), slug: String(e.edition_slug), label: String(e.edition_label), date: String(e.edition_date), periodStart: e.period_start || null, periodEnd: e.period_end || null, entryCount: e.entry_count || 0,
          })),
        };
      }));
      ...
      if (segments.length === 1) {
        ...
        data = {
          program: {
            ...
            latestEdition,
            archive: (editions ?? []).map((e: any) => ({ id: String(e.edition_slug), slug: String(e.edition_slug), label: String(e.edition_label), date: String(e.edition_date), periodStart: e.period_start || null, periodEnd: e.period_end || null, entryCount: e.entry_count || 0 })),
          },
        };
      }
        ...
        const { data: entries } = await supabase
          ...
          .select("id, rank, previous_rank, movement, track_slug, track_title, artist_name, artwork_url, total_score")
