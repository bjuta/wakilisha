...
function summarizeRows(rows: IngestResolvedRow[], methodologyVersion?: string): IngestRunSummary {
...
function getInitialStages(): IngestStageStatus[] {
  return ["validate", "provider_detection", "resource_guard", "source_fetch", "normalize", "canonical_match", "enrichment", "methodology_scoring", "snapshot_commit"].map((stage) => ({ stage: stage as IngestStageStatus["stage"], status: "idle" }));
}
...
const mockRuns: IngestRun[] = [
  ...
  { id: "run-004", chartTitle: "Afrobeats Top 20 — Week 22, 2026", chartSlug: "afrobeats-top-20-week-22-2026", editionDate: "2026-05-30", chartSize: 20, market: "KE", chartKind: "tracks", coverStyle: "genre", sourceUrls: ["https://open.spotify.com/playlist/afrobeats"], detectedProviders: ["spotify"], saveAsRecurringSeries: true, existingSeriesId: "series-afrobeats-20", eligibilityProfileId: "elig_all_artists", methodologyVersion: "top_songs_weighted_v1", status: "failed", stages: [{ stage: "validate", status: "done", durationMs: 100 }, { stage: "provider_detection", status: "done", durationMs: 60 }, { stage: "resource_guard", status: "done", durationMs: 120 }, { stage: "source_fetch", status: "failed", durationMs: 5000, message: "Spotify API rate limit exceeded" }, { stage: "normalize", status: "idle" }, { stage: "canonical_match", status: "idle" }, { stage: "enrichment", status: "idle" }, { stage: "methodology_scoring", status: "idle" }, { stage: "snapshot_commit", status: "idle" }], summary: { totalRows: 0, canonicalMatches: 0, shells: 0, gaps: 0, duplicateCandidates: 0, matchRate: 0, scoringMethodologyVersion: "top_songs_weighted_v1" }, rows: [], createdBy: "Michael", createdAt: "2026-05-30T08:30:00Z", updatedAt: "2026-05-30T08:45:00Z", dryRunCompletedAt: null, committedAt: null, editionId: null, editionSlug: null, snapshotId: null, notes: "", errorMessage: "Spotify API rate limit exceeded during source fetch. Retry after 15 minutes." },
...
export async function runDryRun(request: CreateIngestDryRunRequest): Promise<CreateIngestDryRunResponse> {
  ...
  const run: IngestRun = {
    ...
    summary: { totalRows: 0, canonicalMatches: 0, shells: 0, gaps: 0, duplicateCandidates: 0, matchRate: 0, scoringMethodologyVersion: request.methodologyVersion ?? "top_songs_weighted_v1" },
    rows: [],
    excludedRows: [],
...
async function simulateStageProgress(runId: string): Promise<void> {
  ...
  summary = summarizeRows(rows, methodology.version);
  ...
  const finalStages = [validateStage, providerDetectionStage, resourceGuardStage, sourceFetchStage, normalizeStage, canonicalMatchStage, enrichmentStage, scoringStage, { stage: "snapshot_commit" as const, status: "idle" as const, message: "Ready for commit" }];
  await updateIngestRun(runId, (current) => ({ ...current, methodologyVersion: methodology.version, status: summary.gaps > 0 ? "needs_review" : "dry_run_complete", stages: finalStages, summary, rows, updatedAt: new Date().toISOString(), dryRunCompletedAt: new Date().toISOString() }));
