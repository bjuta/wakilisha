...
// ---- Preset mappings from WP post types to target tables ----
...
const WP_TYPE_TO_TARGET: Record<string, { table: string; label: string; confidence: number; reason: string }> = {
  ...
  wk_artist: { table: "registry_artists", label: "Artists", confidence: 0.95, reason: "Custom post type wk_artist maps to the artist registry." },
  wk_track: { table: "registry_tracks", label: "Tracks", confidence: 0.95, reason: "Custom post type wk_track maps to the track registry." },
  wk_release: { table: "registry_releases", label: "Releases", confidence: 0.95, reason: "Custom post type wk_release maps to the release registry." },
  wk_label: { table: "registry_labels", label: "Labels", confidence: 0.95, reason: "Custom post type wk_label maps to the label registry." },
  wk_genre: { table: "registry_genres", label: "Genres", confidence: 0.95, reason: "Custom post type wk_genre maps to the genre registry." },
...
export async function stageIngestionRun(runId: string): Promise<void> {
  ...
  if (error) throw new Error(`Failed to stage ingestion run: ${error.message}`);
}

export async function finalizeIngestionRun(runId: string, importedCounts: Record<string, number>): Promise<void> {
  const { error } = await supabase
    .from("wk_ingestion_runs")
    .update({
      status: "completed",
  ...
  if (error) throw new Error(`Failed to finalize ingestion run: ${error.message}`);
}