...
const MOVEMENT_CONFIG = {
  up: { icon: "ri-arrow-up-line", color: "var(--wk-success)" },
  down: { icon: "ri-arrow-down-line", color: "var(--wk-danger)" },
  new: { icon: "ri-star-line", color: "var(--wk-brand)" },
  same: { icon: "ri-subtract-line", color: "var(--wk-text-faint)" },
};
...
export function ChartRow({
  ...
  return (
    <div className="group flex items-center gap-3 rounded-lg px-3 py-3 transition-colors hover:bg-[var(--wk-surface-raised)]">
      <div className="flex w-8 shrink-0 flex-col items-center">
        <span
          className="text-lg font-black leading-none"
          style={{ color: rank <= 3 ? "var(--wk-brand)" : "var(--wk-text-muted)" }}
        >
          {rank}
        </span>
        {mvt && (
