...
/* ── Map PublicTaxonomyArticle → MagazineArticle shape ── */
function toMagazineStory(article: PublicTaxonomyArticle, topicName: string) {
  return {
    slug: article.slug,
    title: article.title,
...
export default function TagDetail() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const termData = await fetchTaxonomyTerm("post_tag", slug);
      if (!termData) {
  ...
  const topicName = term?.name || "Tag";
  const stories = useMemo(() => articles.map((a) => toMagazineStory(a, topicName)), [articles, topicName]);
      ...
      <main className="min-h-screen bg-[var(--wk-bg)]">
        <div className="relative min-h-[65vh] flex items-end bg-[#0a0a0a] overflow-hidden -mt-16">
          ...
          <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pb-14 lg:px-8 lg:pb-18">
            <div className="h-4 w-16 rounded bg-white/10 animate-pulse mb-4" />
            <div className="h-14 w-72 rounded bg-white/10 animate-pulse mb-4" />
            <div className="h-5 w-[420px] rounded bg-white/10 animate-pulse" />
          </div>
        </div>
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-[var(--wk-bg)]">
        <div className="max-w-md mx-auto rounded-2xl border border-[var(--wk-border)] bg-[var(--wk-surface)] p-8 text-center">
          ...
          <h1 className="text-[20px] font-black text-[var(--wk-text)] mb-2">Tag not found</h1>
          <p className="text-[13px] text-[var(--wk-text-muted)] mb-6">{error || "This tag does not exist."}</p>
          <Link to="/tags" className="inline-flex items-center gap-2 rounded-full bg-[var(--wk-brand)] text-[var(--wk-brand-on)] px-5 py-2.5 text-[13px] font-bold whitespace-nowrap">
            <i className="ri-arrow-left-line" /> All tags
          </Link>
        </div>
      </main>
    );
      ...
      >
        ...
        {heroStory?.heroUrl ? (
          ...
          <div className="absolute inset-0">
            <div className="absolute inset-0 bg-gradient-to-br from-[#1a1a1f] via-[#14141a] to-[#141a14]" />
            <div className="absolute top-0 right-0 w-[500px] h-[500px] rounded-full bg-emerald-500 opacity-[0.04] blur-[80px]" />
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-black/30 to-black/90" />

        <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pb-14 lg:px-8 lg:pb-18 text-white">
          <Link to="/tags" className="inline-flex items-center gap-2 text-[11px] font-extrabold uppercase tracking-[0.22em] text-white/50 hover:text-white/80 transition-colors mb-5">
            <span className="h-px w-5 bg-white/30" /> Tags
          </Link>
          <div className="flex items-center gap-3 mb-2">
            <span className="inline-flex items-center rounded-full bg-white/10 backdrop-blur-md border border-white/15 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.14em] text-white/50">
              <i className="ri-price-tag-3-line text-[11px] mr-1" /> Topic
            </span>
          </div>
      ...
      <div className="max-w-[1280px] mx-auto px-6 lg:px-8 flex flex-col gap-20 py-16">
        ...
        {stories.length === 0 && (
          <section className="mag-reveal text-center py-16">
            ...
            <p className="text-[13px] text-[var(--wk-text-muted)]">
              No stories are tagged with &ldquo;{term.name}&rdquo; yet.
            </p>
          </section>
        )}

        {/* ── Stories (dynamic asymmetry: 1 hero + 3 compact stacked) ── */}
        {stories.length > 0 && heroStory && (
          <section className="mag-reveal">
            <SectionLabel count={stories.length}>Stories tagged &ldquo;{term.name}&rdquo;</SectionLabel>
        ...
        <footer className="border-t border-[var(--wk-border)] pt-14 pb-8 text-center">
          ...
          <p className="text-[24px] lg:text-[28px] font-black tracking-[-0.035em] text-[var(--wk-text)] leading-snug max-w-[420px] mx-auto">
            {totalCount} {totalCount === 1 ? "story" : "stories"} tagged &ldquo;{term.name}&rdquo;.
          </p>
        </footer>
      </div>
    </main>
