...
type ScanEvidence = { csv_headers?: Array<{ path: string; kind: string; headers: string[]; rows: number }>; json_keys?: Array<{ path: string; kind: string; keys: string[] }>; wxr_post_types?: Record<string, number>; sql_tables?: string[] };
type ScanManifest = { scanned_at?: string; archive?: { file_count?: number; total_uncompressed_bytes?: number }; counts?: Record<string, number>; detected?: string[]; files?: Array<{ path: string; size: number; extension: string; kind: string; rows?: number; headers?: string[]; warning?: string }>; evidence?: ScanEvidence; warnings?: string[] };
...
type MappingManifest = { discovered_at?: string; processor?: string; version?: string; summary?: { total?: number; auto_matched?: number; needs_review?: number }; candidates?: MappingCandidate[] };
type StagingBucket = { target_entity: string; candidate_count: number; auto_matched: number; needs_review: number; blocked: boolean; blocker_reasons: string[]; source_files: string[]; candidate_ids: string[] };
type StagingPlan = { planned_at?: string; processor?: string; version?: string; mode?: string; scan_summary?: { archive_files?: number; count_groups?: number }; mapping_summary?: { total_candidates?: number; auto_matched?: number; needs_review?: number }; buckets?: StagingBucket[]; readiness?: { ready_bucket_count?: number; blocked_bucket_count?: number; can_stage_anything?: boolean; can_import_to_production?: boolean; reason?: string }; blockers?: string[] };

type ActiveTab = "overview" | "scan" | "mappings" | "plan" | "manifest" | "warnings" | "errors";

function getScan(run: IngestionRun): ScanManifest | null { const scan = run.source_manifest?.scan; return scan && typeof scan === "object" ? scan as ScanManifest : null; }
function getMappings(run: IngestionRun): MappingManifest | null { const mappings = run.source_manifest?.mappings; return mappings && typeof mappings === "object" ? mappings as MappingManifest : null; }
function getStagingPlan(run: IngestionRun): StagingPlan | null { const plan = run.source_manifest?.staging_plan; return plan && typeof plan === "object" ? plan as StagingPlan : null; }
...
export default function AdminImportsJobDetailPage() {
  ...
  const planBucketCount = plan?.buckets?.length ?? 0;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-1 border-b border-wk-border">
        {[
          ...
          { key: "plan", label: "Staging Plan", icon: "ListChecks", count: planBucketCount },
...
function OverviewTab({ run, scan, mappings, plan }: { run: IngestionRun; scan: ScanManifest | null; mappings: MappingManifest | null; plan: StagingPlan | null }) {
  const counts = run.imported_counts ?? {};
  return <div className="grid gap-5 lg:grid-cols-[1fr_320px]"><div className="space-y-4"><WkSurface className="p-5"><h3 className="mb-4 text-[14px] font-bold text-wk-text">Imported Records</h3>{Object.keys(counts).length > 0 ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{Object.entries(counts).map(([key, count]) => <Metric key={key} label={key} value={count} />)}</div> : <HonestEmpty body="No imported counts have been written by the backend processor yet." />}</WkSurface><WkSurface className="p-5"><h3 className="mb-4 text-[14px] font-bold text-wk-text">Real Scan / Mapping / Plan Progress</h3><div className="grid gap-3 sm:grid-cols-4"><Metric label="Scanned files" value={scan?.archive?.file_count ?? 0} /><Metric label="Count groups" value={Object.keys(scan?.counts ?? {}).length} /><Metric label="Mappings" value={mappings?.candidates?.length ?? 0} /><Metric label="Plan buckets" value={plan?.buckets?.length ?? 0} /></div></WkSurface></div><div className="space-y-4"><WkSurface className="p-5"><h3 className="mb-3 text-[14px] font-bold text-wk-text">Job Info</h3><div className="space-y-3"><InfoRow label="ID" value={run.id} /><InfoRow label="Source" value={run.source_name} /><InfoRow label="Kind" value={run.source_kind} /><InfoRow label="Status" value={run.status} /><InfoRow label="Created" value={new Date(run.created_at).toLocaleString()} />{run.started_at && <InfoRow label="Started" value={new Date(run.started_at).toLocaleString()} />}{run.finished_at && <InfoRow label="Finished" value={new Date(run.finished_at).toLocaleString()} />}</div></WkSurface></div></div>;
}

function ScanTab({ scan }: { scan: ScanManifest | null }) { if (!scan) return <HonestEmpty body="No scan has been written to source_manifest.scan yet. Run the real ZIP processor first." />; const counts = scan.counts ?? {}; const csvEvidence = scan.evidence?.csv_headers ?? []; const jsonEvidence = scan.evidence?.json_keys ?? []; const wxrPostTypes = scan.evidence?.wxr_post_types ?? {}; const sqlTables = scan.evidence?.sql_tables ?? []; return <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-3"><Metric label="Files" value={scan.archive?.file_count ?? 0} /><Metric label="Uncompressed MB" value={Math.round((scan.archive?.total_uncompressed_bytes ?? 0) / 1024 / 1024)} /><Metric label="Detected groups" value={(scan.detected ?? []).length} /></div><WkSurface className="p-5"><h3 className="mb-4 text-[14px] font-bold text-wk-text">Counts from real scan</h3>{Object.keys(counts).length ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{Object.entries(counts).map(([key, count]) => <Metric key={key} label={key} value={count} />)}</div> : <HonestEmpty body="The scan did not produce count groups." />}</WkSurface><WkSurface className="p-5"><h3 className="mb-4 text-[14px] font-bold text-wk-text">Detected evidence</h3><div className="space-y-4"><EvidenceBlock title="CSV headers" rows={csvEvidence.map((item) => ({ label: `${item.kind} · ${item.rows} rows`, body: `${item.path}\n${item.headers.join(', ')}` }))} /><EvidenceBlock title="JSON keys" rows={jsonEvidence.map((item) => ({ label: item.kind, body: `${item.path}\n${item.keys.join(', ')}` }))} /><EvidenceBlock title="WXR post types" rows={Object.entries(wxrPostTypes).map(([key, value]) => ({ label: key, body: `${value} items` }))} /><EvidenceBlock title="SQL tables" rows={sqlTables.map((table) => ({ label: table, body: "Detected in SQL dump" }))} /></div></WkSurface></div>; }

function MappingsTab({ mappings }: { mappings: MappingManifest | null }) { const candidates = mappings?.candidates ?? []; if (!mappings) return <HonestEmpty body="No mappings have been written to source_manifest.mappings yet. Run the real mapping discovery processor first." />; return <div className="space-y-5"><WkSurface className="p-5"><div className="grid gap-3 sm:grid-cols-3"><Metric label="Total" value={mappings.summary?.total ?? candidates.length} /><Metric label="Auto matched" value={mappings.summary?.auto_matched ?? candidates.filter((item) => item.status === 'auto_matched').length} /><Metric label="Needs review" value={mappings.summary?.needs_review ?? candidates.filter((item) => item.status === 'needs_review').length} /></div></WkSurface><div className="space-y-3">{candidates.map((item) => <div key={item.id} className="rounded-2xl border border-wk-border bg-wk-bg-subtle p-4"><div className="grid gap-4 lg:grid-cols-[1fr_auto_1fr] lg:items-center"><div><p className="text-[10px] font-black uppercase tracking-wider text-wk-text-muted">Source</p><h3 className="mt-1 text-[14px] font-black text-wk-text">{item.source.entity}.{item.source.field}</h3><p className="mt-1 whitespace-pre-wrap text-[11px] text-wk-text-muted">{item.source.evidence}</p></div><div className="hidden text-wk-brand lg:block">→</div><div><p className="text-[10px] font-black uppercase tracking-wider text-wk-text-muted">Target</p><h3 className="mt-1 text-[14px] font-black text-wk-text">{item.target.entity}.{item.target.field}</h3><p className="mt-1 text-[11px] text-wk-text-muted">{item.reason}</p></div></div><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full bg-wk-surface px-3 py-1 text-[11px] font-black text-wk-text-muted">{Math.round(item.confidence * 100)}% confidence</span><span className={`rounded-full px-3 py-1 text-[11px] font-black uppercase ${item.status === 'auto_matched' ? 'bg-wk-success-soft text-wk-success' : 'bg-wk-warning-soft text-wk-warning'}`}>{item.status.replace('_', ' ')}</span></div></div>)}</div></div>; }

function StagingPlanTab({ plan }: { plan: StagingPlan | null }) {
  if (!plan) return <HonestEmpty body="No staging plan has been written to source_manifest.staging_plan yet. Run imports:plan-wordpress-staging first." />;
  const buckets = plan.buckets ?? [];
  return <div className="space-y-5"><WkSurface className="p-5"><h3 className="mb-4 text-[14px] font-bold text-wk-text">Plan Readiness</h3><div className="grid gap-3 sm:grid-cols-4"><Metric label="Buckets" value={buckets.length} /><Metric label="Ready" value={plan.readiness?.ready_bucket_count ?? 0} /><Metric label="Blocked" value={plan.readiness?.blocked_bucket_count ?? 0} /><Metric label="Production import" value={plan.readiness?.can_import_to_production ? 1 : 0} /></div><p className="mt-4 text-[12px] leading-5 text-wk-text-muted">{plan.readiness?.reason ?? "This is a staging plan only."}</p></WkSurface>{(plan.blockers ?? []).length > 0 && <ListPanel title="Blockers" icon="AlertTriangle" tone="warning" items={plan.blockers ?? []} empty="No blockers recorded." />}<div className="space-y-3">{buckets.map((bucket) => <div key={bucket.target_entity} className="rounded-2xl border border-wk-border bg-wk-bg-subtle p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-wider text-wk-text-muted">Target entity</p><h3 className="mt-1 text-[18px] font-black text-wk-text">{bucket.target_entity}</h3><p className="mt-2 text-[12px] text-wk-text-muted">{bucket.candidate_count} candidates · {bucket.auto_matched} auto-matched · {bucket.needs_review} needs review</p></div><span className={`rounded-full px-3 py-1 text-[11px] font-black uppercase ${bucket.blocked ? 'bg-wk-warning-soft text-wk-warning' : 'bg-wk-success-soft text-wk-success'}`}>{bucket.blocked ? 'blocked' : 'ready'}</span></div>{bucket.blocker_reasons.length > 0 && <div className="mt-4 space-y-2">{bucket.blocker_reasons.map((reason) => <div key={reason} className="rounded-lg border border-wk-warning/20 bg-wk-warning-soft p-3 text-[12px] text-wk-warning">{reason}</div>)}</div>}{bucket.source_files.length > 0 && <details className="mt-4"><summary className="cursor-pointer text-[12px] font-black text-wk-text">Source files</summary><pre className="mt-2 whitespace-pre-wrap rounded-lg border border-wk-border bg-wk-surface p-3 text-[11px] text-wk-text-muted">{bucket.source_files.join('\n')}</pre></details>}</div>)}</div></div>;
}
...
function StatusBadge({ status }: { status: string }) { const color = status === "completed" ? "bg-wk-success-soft text-wk-success" : RUNNING_STATUSES.has(status) ? "bg-wk-info-soft text-wk-info" : status === "failed" ? "bg-wk-danger-soft text-wk-danger" : status === "queued" ? "bg-wk-warning-soft text-wk-warning" : status === "mapped" || status === "scanned" || status === "planned" ? "bg-wk-brand-soft text-wk-brand" : "bg-wk-surface-raised text-wk-text-muted"; return <span className={`inline-flex items-center rounded-full px-3 py-1 text-[11px] font-bold uppercase ${color}`}>{status}</span>; }
