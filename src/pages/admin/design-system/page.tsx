...
import {
  wakilishaDesignChapters,
  wakilishaParityPageMap,
  wakilishaDesignSystemMeta,
} from "../../../design-system/designSystemManifest";
...
export default function AdminDesignSystem() {
  ...
  const filteredChapters = useMemo(() => {
    let list = wakilishaDesignChapters;
  ...
  const selectedChapter = useMemo(
    () =>
      wakilishaDesignChapters.find((c) => c.id === selectedChapterId) || null,
    [selectedChapterId],
  );

  const chapterCount = wakilishaDesignChapters.length;
  const implementedCount = 0;
  const parityPageCount = wakilishaParityPageMap.length;

  return (
    <div className="wk-app-shell min-h-screen bg-[var(--wk-bg)] text-[var(--wk-text)]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[13px] font-bold leading-tight tracking-tight text-[var(--wk-text)]">
                {wakilishaDesignSystemMeta.name}
              </h1>
              <p className="text-[11px] text-[var(--wk-text-muted)]">
                {wakilishaDesignSystemMeta.version} — {wakilishaDesignSystemMeta.northStar}
              </p>
            </div>
          </div>
        ...
        <aside className="hidden w-56 shrink-0 lg:block">
          <div className="sticky top-24">
            <div className="mb-4 grid grid-cols-3 gap-2">
              ...
              <WkSurface className="p-3 text-center">
                <div className="text-lg font-bold text-[var(--wk-text-soft)]">{implementedCount}</div>
                <div className="text-[11px] text-[var(--wk-text-muted)]">Done</div>
              </WkSurface>
              <WkSurface className="p-3 text-center">
                <div className="text-lg font-bold text-[var(--wk-text-soft)]">{parityPageCount}</div>
        ...
        <main className="min-w-0 flex-1">
          <div className="mb-6 flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search chapters, rules, parity targets..."
          ...
          {activeSection === "chapters" && (
            <ChapterBrowser
              ...
              allChapters={wakilishaDesignChapters}
              parityMap={wakilishaParityPageMap}
            />
          )}
          ...
          {activeSection === "parity" && (
            <ParityMap
              parityMap={wakilishaParityPageMap}
              chapters={wakilishaDesignChapters}
            />
          )}
