...
export function SpecimenCanvas({ item }: { item: WkRichMediaSpecimen }) {
  const key = `${item.id} ${item.label} ${item.kind} ${item.canonicalClass ?? ''}`.toLowerCase();
  if (key.includes('logo')) return h(LogoBoard);
  if (key.includes('color') || key.includes('swatch') || key.includes('token') || key.includes('contrast')) return h(TokenBoard);
  if (key.includes('type') || key.includes('typography')) return h(TypeBoard);
  if (key.includes('spacing') || key.includes('radius')) return h(SpacingBoard);
  if (key.includes('genre')) return h(GenreBoard);
  if (key.includes('artist')) return h(ArtistBoard);
  if (key.includes('magazine')) return h(MagazineBoard);
  if (key.includes('chart') || key.includes('rank') || key.includes('row')) return h(ChartBoard);
  if (key.includes('single track') || key.includes('waveform')) return h(TrackBoard);
  if (key.includes('album') || key.includes('release')) return h(AlbumBoard);
  if (key.includes('modal') || key.includes('overlay')) return h(ModalBoard);
  if (key.includes('label')) return h(LabelBoard);
  if (key.includes('article') || key.includes('post')) return h(ArticleBoard);
  if (key.includes('share') || key.includes('og')) return h(ShareBoard);
  if (key.includes('mobile') || key.includes('phone')) return h(MobileBoard);
  if (key.includes('player') || key.includes('dock') || key.includes('sheet') || key.includes('theater')) return h(PlayerBoard);
  if (key.includes('motion') || key.includes('animation') || key.includes('pulse') || key.includes('shimmer')) return h(MotionBoard);
  if (key.includes('admin')) return h(AdminBoard);
  return h(CardBoard);
}
