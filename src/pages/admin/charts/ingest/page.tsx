...
export default function AdminChartsIngest() {
  ...
  async function handleCancelRun(runId: string) {
    ...
    try { await cancelIngestRun(runId); await loadData(); } catch (err) { setFormError(err instanceof Error ? err.message : "Cancel failed"); } finally { setCancelLoading(null); }
  }

  async function handleRetryRun(runId: string) {
    setRetryLoading(runId);
    try { await retryIngestRun(runId); await loadData(); } catch (err) { setFormError(err instanceof Error ? err.message : "Retry failed"); } finally { setRetryLoading(null); }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold ${mode === "mock" ? "bg-wk-warning-soft text-wk-warning" : "bg-wk-success-soft text-wk-success"}`}>
            <WkIcon name={mode === "mock" ? "FlaskConical" : "Globe"} size={12} />
            {mode === "mock" ? "Mock Mode" : "WordPress Mode"}
          </span>
          <button onClick={() => navigate("/admin/charts/ingest-runs")} className={BTN_GHOST}>
            <WkIcon name="List" size={14} />All Runs
          </button>
          <button onClick={() => navigate("/admin/charts/ingest-health")} className={BTN_GHOST}>
            <WkIcon name="HeartPulse" size={14} />API Health
          </button>
        </div>
      </div>
      ...
      {formError && (
        <div className="rounded-lg border border-wk-danger/20 bg-wk-danger-soft p-3">
          <div className="flex items-center gap-2 text-wk-danger">
      ...
      {successMessage && (
        <div className="rounded-lg border border-wk-success/20 bg-wk-success-soft p-3">
          <div className="flex items-center gap-2 text-wk-success">
      ...
      {editionExistsWarning && (
        <div className="rounded-lg border border-wk-warning/20 bg-wk-warning-soft p-3">
          <div className="flex items-center gap-2 text-wk-warning">
          ...
          {step === "configure" && (
            <WkSurface className="p-5">
              ...
              <div className="mb-4">
                ...
                <select value={existingSeriesId} onChange={(e) => setExistingSeriesId(e.target.value)} className={INPUT_CLASS}>
                  <option value="">— Select a series —</option>
                  {families.map((f) => <option key={f.id} value={f.id}>{f.label} ({f.familyKey})</option>)}
                ...
                {existingSeriesId === "__new__" && (
                  <div className="mt-2 rounded-lg border border-wk-brand/20 bg-wk-brand-soft p-3">
                    <p className="text-[12px] font-semibold text-wk-brand mb-2">Create New Series</p>
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                      <input type="text" value={newSeriesLabel} onChange={(e) => { setNewSeriesLabel(e.target.value); setNewSeriesKey(e.target.value.toLowerCase().replace(/[^a-z0-9]/g, "")); }} placeholder="Series name" className={INPUT_CLASS} />
                      <input type="text" value={newSeriesKey} onChange={(e) => setNewSeriesKey(e.target.value)} placeholder="series-key" className={INPUT_CLASS} />
                    </div>
                    <div className="mt-2 flex gap-2">
                      <button
                        onClick={() => {
                          if (newSeriesLabel && newSeriesKey) {
                            const newFamily: ChartFamily = { id: newSeriesKey, familyKey: newSeriesKey, label: newSeriesLabel, description: "Custom chart series", defaultChartSize: chartSize, defaultRegion: market, editionFrequency: "weekly", defaultRuleset: "csv_registry_import_v1", defaultScoringModel: "csv_position_order", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-4">
                <div>
                  ...
                  <input type="text" value={chartTitle} onChange={(e) => setChartTitle(e.target.value)} placeholder="e.g. WAKILISHA Top 40 — Week 22" className={INPUT_CLASS} />
                </div>
                <div>
                  <label className={LABEL_CLASS}>Chart Slug *</label>
                  <input type="text" value={chartSlug} onChange={(e) => setChartSlug(e.target.value)} placeholder="wakilisha-top-40-week-22" className={INPUT_CLASS} />
                  {publicUrlPreview && (
                    <p className="mt-1 text-[11px] text-wk-text-muted">Public URL: <span className="font-semibold text-wk-brand">{publicUrlPreview}</span></p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-4">
                <div>
                  ...
                  <input type="date" value={editionDate} onChange={(e) => setEditionDate(e.target.value)} className={INPUT_CLASS} />
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input type="range" min={10} max={100} step={10} value={chartSize} onChange={(e) => setChartSize(Number(e.target.value))} className="flex-1 accent-wk-brand" />
                    <span className="w-10 text-right text-[13px] font-bold text-wk-text">{chartSize}</span>
                  </div>
                </div>
                <div>
                  <label className={LABEL_CLASS}>Market *</label>
                  <select value={market} onChange={(e) => setMarket(e.target.value)} className={INPUT_CLASS}>
              ...
              <div className="mb-4">
                ...
                <textarea value={sourceUrls} onChange={(e) => setSourceUrls(e.target.value)} rows={3} placeholder="https://open.spotify.com/playlist/...&#10;https://music.apple.com/..." className={`${INPUT_CLASS} resize-none`} />
                {detectedProviders.length > 0 && (
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    {detectedProviders.map((p) => <ProviderChip key={p} provider={p} />)}
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-4">
                ...
                <div>
                  ...
                  <select value={coverStyle} onChange={(e) => setCoverStyle(e.target.value)} className={INPUT_CLASS}>
              ...
              <div className="mb-5">
                <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-wk-border bg-wk-surface px-3 py-2 hover:bg-wk-surface-raised transition-colors w-fit">
                  <input type="checkbox" checked={saveAsRecurring} onChange={(e) => setSaveAsRecurring(e.target.checked)} className="h-4 w-4 rounded border-wk-border accent-wk-brand" />
            ...
            <div className="space-y-5">
              ...
              <WkSurface className="p-4 overflow-hidden">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[12px] text-wk-text-muted">Top {Math.min(10, dryRunResult.rows.length)} of {dryRunResult.rows.length}</span>
                </div>
                <div className="space-y-1">
                  {dryRunResult.rows.slice(0, 10).map((row, idx) => <MiniChartRow key={row.id} row={row} index={idx} />)}
                </div>
              </WkSurface>
              <WkSurface className="overflow-hidden">
                <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[12px] text-wk-text-muted">{dryRunResult.rows.length} rows</span>
                    <div className="flex flex-wrap gap-1">
                      {["all", "canonical", "shell", "no_match", "needs_review", "duplicate_candidate"].map((f) => (
                        <button key={f} onClick={() => setRowFilter(f)} className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold transition-all ${rowFilter === f ? "bg-wk-brand text-wk-brand-on" : "bg-wk-surface-raised text-wk-text-soft hover:bg-wk-border"}`}>
                ...
                {filteredRows.length === 0 && (
                  <div className="px-4 py-8 text-center text-[13px] text-wk-text-muted">No rows match the selected filter.</div>
                )}
              </WkSurface>
        ...
        <div className="space-y-5">
          ...
          {activeRun && !dryRunResult && (
            <WkSurface className="p-4 border-l-4 border-l-wk-brand">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="flex items-center gap-1 text-[11px] font-semibold text-wk-brand">
                  <span className="inline-block h-2 w-2 rounded-full bg-wk-brand animate-pulse" />In progress
                </span>
              </div>
              <PipelinePanel run={activeRun} compact />
              <button onClick={() => navigate(`/admin/charts/ingest-runs/${activeRun.id}`)} className="mt-3 w-full rounded-md bg-wk-brand-soft px-3 py-2 text-[12px] font-semibold text-wk-brand transition-colors hover:bg-wk-brand/20">
                Monitor Run →
              </button>
            </WkSurface>
          )}
          ...
          <WkSurface className="p-4">
            ...
            <div className="space-y-2">
              {runs.filter((r) => r.status === "running" || r.status === "draft" || r.status === "dry_run_complete").slice(0, 4).map((run) => (
                <RunCard key={run.id} run={run} onClick={() => navigate(`/admin/charts/ingest-runs/${run.id}`)} onCancel={() => handleCancelRun(run.id)} onRetry={() => handleRetryRun(run.id)} cancelLoading={cancelLoading === run.id} retryLoading={retryLoading === run.id} />
              ))}
              {runs.filter((r) => r.status === "running" || r.status === "draft" || r.status === "dry_run_complete").length === 0 && (
                <p className="text-[13px] text-wk-text-muted">No active runs</p>
              )}
            </div>
          </WkSurface>
          ...
          <WkSurface className="p-4">
            ...
            <div className="space-y-1">
              <NavButton icon={GitPullRequest} label="Review Queue" path="/admin/charts/review-queue" />
              <NavButton icon={XCircle} label="No-match Releases" path="/admin/charts/no-match" />
              <NavButton icon={FolderPlus} label="Release Shells" path="/admin/charts/release-shells" />
              <NavButton icon={AlertCircle} label="Canon Gaps" path="/admin/charts/canon-gaps" />
              <NavButton icon={History} label="Legacy Ingest Jobs" path="/admin/charts/ingest-jobs" />
            </div>
          </WkSurface>
        </div>
      </div>
