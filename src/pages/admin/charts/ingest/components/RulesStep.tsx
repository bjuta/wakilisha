...
import { EnrichmentOptionsStep } from "./EnrichmentOptionsStep";
...
export function RulesStep({ profiles, selectedEligibilityProfileId, onSelectEligibilityProfile, onBack, onContinue }: RulesStepProps) {
  ...
  const [enrichmentOptions, setEnrichmentOptions] = useState<IngestEnrichmentOptions>(DEFAULT_INGEST_ENRICHMENT_OPTIONS);
    ...
    try {
      const created = createEligibilityProfile({
        ...
        trackEligibility: {
          explicitAllowed,
          requireIsrc,
          requirePreview: requirePreview || enrichmentOptions.requirePreview,
        },
        collaborationRules: {
    ...
    <WkSurface className="p-5">
      <div className="mb-5 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[12px] text-wk-text-soft">
            Build eligibility like Jenga: choose origin, artist type, gender, release window, collaboration policy, review rules, and enrichment policy. The result becomes a saved profile attached to this ingest run.
          </p>
        </div>
      ...
      <div className="mb-5 rounded-xl border border-wk-border bg-wk-surface-raised p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div><label className={LABEL_CLASS}>Profile name *</label><input value={profileName} onChange={(event) => { setProfileName(event.target.value); setProfileSlug(slugify(event.target.value)); }} className={INPUT_CLASS} placeholder="Kenyan female tracks Q1 2026" /></div>
          <div><label className={LABEL_CLASS}>Slug *</label><input value={profileSlug} onChange={(event) => setProfileSlug(event.target.value)} className={INPUT_CLASS} placeholder="kenyan-female-tracks-q1-2026" /></div>
          <div><label className={LABEL_CLASS}>Public visibility</label><WkSelect value={visibility} onChange={(value) => setVisibility(value as "public" | "admin_only")} triggerClassName={INPUT_CLASS}><option value="admin_only">Admin-only rules</option><option value="public">Can be shown publicly</option></WkSelect></div>
          <div><label className={LABEL_CLASS}>Unknown metadata policy</label><WkSelect value={unknownMetadataPolicy} onChange={(value) => setUnknownMetadataPolicy(value as "warning" | "review")} triggerClassName={INPUT_CLASS}><option value="review">Send unknowns to review</option><option value="warning">Allow unknowns with warning</option></WkSelect></div>
        </div>
      </div>

      <div className="mb-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-wk-border bg-wk-surface-raised p-4"><h3 className="mb-3 text-[13px] font-bold text-wk-text">Artist origin</h3><WkSelect value={originMode} onChange={(value) => setOriginMode(value as "any" | "country_only" | "multi_country")} triggerClassName={INPUT_CLASS}><option value="any">Any origin</option><option value="country_only">One country only</option><option value="multi_country">Selected countries</option></WkSelect>{originMode !== "any" && <div className="mt-3 flex flex-wrap gap-2">{COUNTRY_OPTIONS.map((country) => <button key={country} type="button" onClick={() => toggleCountry(country)} className={`rounded-full border px-3 py-1 text-[11px] font-semibold ${originCountries.includes(country) ? "border-wk-brand bg-wk-brand text-wk-brand-on" : "border-wk-border bg-wk-surface text-wk-text-soft"}`}>{country}</button>)}</div>}</div>
        <div className="rounded-xl border border-wk-border bg-wk-surface-raised p-4"><h3 className="mb-3 text-[13px] font-bold text-wk-text">Artist classification</h3><div className="grid gap-3 sm:grid-cols-2"><div><label className={LABEL_CLASS}>Gender</label><WkSelect value={genderMode} onChange={(value) => setGenderMode(value as ArtistGenderEligibilityMode)} triggerClassName={INPUT_CLASS}><option value="any">Any</option><option value="female_only">Female artists only</option><option value="male_only">Male artists only</option><option value="mixed_gender_only">Mixed gender only</option><option value="non_binary_inclusive">Non-binary inclusive</option></WkSelect></div><div><label className={LABEL_CLASS}>Artist type</label><WkSelect value={artistTypeMode} onChange={(value) => setArtistTypeMode(value as ArtistTypeEligibilityMode)} triggerClassName={INPUT_CLASS}><option value="any">Any</option><option value="solo_artists_only">Solo artists only</option><option value="groups_collectives_only">Groups/collectives only</option><option value="bands_only">Bands only</option><option value="duos_only">Duos only</option></WkSelect></div></div></div>
      </div>

      <div className="mb-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-wk-border bg-wk-surface-raised p-4"><h3 className="mb-3 text-[13px] font-bold text-wk-text">Track/release eligibility</h3><p className="mb-3 text-[11px] text-wk-text-muted">Release window: only tracks released within these dates will be eligible.</p><div className="mb-3"><WkCheckbox checked={enableReleaseWindow} onChange={(checked) => setEnableReleaseWindow(checked)} className="flex w-fit cursor-pointer items-center gap-2 rounded-lg border border-wk-border bg-wk-surface px-3 py-2 transition-colors hover:bg-wk-surface-raised"><span className="text-[12px] font-semibold text-wk-text-soft">Enable release window</span></WkCheckbox>{enableReleaseWindow && <div className="mt-3"><DateRangePicker value={releaseWindowRange} onChange={setReleaseWindowRange} presets={[]} /></div>}</div><div className="mt-3 flex flex-wrap gap-2">{RELEASE_TYPES.map((type) => <button key={type} type="button" onClick={() => toggleReleaseType(type)} className={`rounded-full border px-3 py-1 text-[11px] font-semibold ${releaseTypes.includes(type) ? "border-wk-brand bg-wk-brand text-wk-brand-on" : "border-wk-border bg-wk-surface text-wk-text-soft"}`}>{type}</button>)}</div></div>
      ...
      <div className="mb-5">
        <EnrichmentOptionsStep options={enrichmentOptions} onChange={setEnrichmentOptions} />
      </div>

      <div className="mb-5 rounded-xl border border-wk-border bg-wk-bg p-4"><div className="mb-2 flex items-center gap-2 text-[12px] font-bold text-wk-text"><WkIcon name="ClipboardCheck" size={14} className="text-wk-brand" /> Built rule summary</div><p className="text-[12px] text-wk-text-soft">{buildDescription()}</p>{selectedProfile && <p className="mt-2 text-[11px] text-wk-text-muted">Currently selected saved profile: <strong className="text-wk-text-soft">{selectedProfile.name}</strong></p>}</div>
