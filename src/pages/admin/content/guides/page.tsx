...
export default function AdminGuidesPage() {
  ...
  const filtered = guides.filter((g) => {
    const matchesSearch =
      !search ||
      (g.title?.toLowerCase().includes(search.toLowerCase()) ?? false) ||
      g.slug.toLowerCase().includes(search.toLowerCase());
    const matchesStatus = statusFilter === "all" || g.wp_status === statusFilter;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="text-[22px] font-black tracking-tight text-wk-text">Guides</h1>
          <p className="mt-1 text-[13px] text-wk-text-muted">
            {guides.length} guides in the system.
          </p>
        </div>
