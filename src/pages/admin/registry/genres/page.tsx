  ...
  useEffect(() => {
    async function load() {
      const { data, error } = await supabase
        ...
        .select("slug, name, parent_genre_id, description, status, created_at, updated_at")
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="text-[22px] font-black tracking-tight text-wk-text">Genres</h1>
          <p className="mt-1 text-[13px] text-wk-text-muted">
            {genres.length} genres in registry.
          </p>
        </div>
      ...
      <WkSurface className="p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] text-wk-text-muted whitespace-nowrap">{filtered.length} of {genres.length}</span>
          </div>
        </div>
      </WkSurface>
      ...
      {loading ? (
        ...
        <AdminTable
          columns={[
            ...
            { key: "parent_genre_id", label: "Parent", width: "140px", render: (row) => <span className="text-[12px] text-wk-text-muted">{row.parent_genre_id ? "Has parent" : "—"}</span> },
          ...
          onRowClick={(row) => navigate(`/admin/registry/genres/${row.slug}`)}
        />
      )}
    </div>
  );
