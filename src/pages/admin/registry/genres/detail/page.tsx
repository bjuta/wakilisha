...
export default function GenreDetailPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error } = await supabase
        ...
        .select("id, slug, name, parent_genre_id, description, status, metadata, created_at, updated_at")
        .eq("slug", slug)
        .maybeSingle();
      if (error) {
        addToast("error", "Failed to load genre.");
        setLoading(false);
        return;
      }
      if (!data) {
      ...
      setDraft({
        name: data.name,
        parent_genre_id: data.parent_genre_id ?? "",
  ...
  async function handleDelete() {
    ...
    setTimeout(() => navigate("/admin/registry/genres"), 1000);
  }
  ...
  const { relations, loading: relLoading } = useRelatedEntities("genre", slug);
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-[11px] text-wk-text-faint mb-1.5">
            <button onClick={() => navigate("/admin/registry/genres")} className="text-wk-brand hover:text-wk-brand-hover font-black uppercase tracking-wider transition-colors">Genres</button>
          ...
          <p className="mt-1 text-[12px] text-wk-text-faint font-mono">{genre.slug}</p>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
        <div className="space-y-4">
          ...
          <WkSurface className="p-5">
            ...
            <textarea value={draft.description} onChange={(e) => patchDraft({ description: e.target.value })} placeholder="Genre description..." rows={6} className="w-full rounded-lg border border-wk-border bg-wk-bg-subtle px-4 py-3 text-[13px] text-wk-text placeholder:text-wk-text-faint outline-none focus:border-wk-brand resize-none transition-colors" />
          </WkSurface>
        </div>
        ...
        <div className="space-y-4">
          <WkSurface className="p-4">
            ...
            <div className="space-y-2">
              <InfoRow label="Slug" value={genre.slug} mono />
              <InfoRow label="Parent ID" value={genre.parent_genre_id ?? "—"} mono />
              <InfoRow label="Created" value={new Date(genre.created_at).toLocaleString()} />
              <InfoRow label="Modified" value={new Date(genre.updated_at).toLocaleString()} />
            </div>
          </WkSurface>
