...
const SETTINGS_ITEMS: SettingsNavItem[] = SETTINGS_DOMAINS.map((d) => ({
  path: d.route,
  label: d.label,
  icon: d.icon as WkIconName,
}));
...
export function AdminSettingsLayout() {
  ...
  const getPageTitle = () => {
    const match = SETTINGS_ITEMS.find((i) => isActive(i.path));
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex h-14 items-center gap-3 border-b border-[var(--wk-border)] px-3">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[var(--wk-brand)] text-[var(--wk-brand-on)]">
          <WkIcon name="Settings" size={16} />
        </div>
        <div className="min-w-0 overflow-hidden">
          <h1 className="truncate text-[13px] font-black tracking-tight text-[var(--wk-text)]">WAKILISHA</h1>
          <p className="truncate text-[10px] text-[var(--wk-text-muted)]">Settings</p>
        </div>
      </div>

      {/* Nav Groups */}
      <div className="flex-1 overflow-y-auto py-3">
        <div className="mb-2 px-3 text-[10px] font-bold uppercase tracking-wider text-[var(--wk-text-faint)]">Platform Settings</div>
        <div className="space-y-0.5 px-2">
          <SidebarLink
            item={{ path: "/admin/settings", label: "Settings Hub", icon: "LayoutDashboard" }}
            active={isActive("/admin/settings")}
            onClick={() => { navigate("/admin/settings"); setMobileOpen(false); }}
          />
        </div>

        <div className="mb-2 mt-4 px-3 text-[10px] font-bold uppercase tracking-wider text-[var(--wk-text-faint)]">Domains</div>
        <div className="space-y-0.5 px-2">
          {SETTINGS_ITEMS.map((item) => (
            <SidebarLink key={item.path} item={item} active={isActive(item.path)} onClick={() => { navigate(item.path); setMobileOpen(false); }} />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--wk-bg)] text-[var(--wk-text)]">
      {/* Desktop Sidebar */}
      <aside className="hidden shrink-0 flex-col border-r border-[var(--wk-border)] bg-[var(--wk-surface)] transition-all duration-300 ease-[var(--wk-ease-snap)] md:flex w-[260px]">
        {sidebarContent}
      </aside>
          ...
          <div className="flex flex-col items-center justify-between gap-2 sm:flex-row">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-[var(--wk-text-muted)]">Platform settings</p>
              </div>
            </div>
