...
function buildMockSuggestions(row: RegistryReleaseShell): MockEnrichmentSuggestion[] {
  return [
    ...
    {
      id: `${row.shellKey}-artist_display_name`,
    ...
    {
      id: `${row.shellKey}-source_surface`,
...
export default function AdminSettingsRegistryReleaseShells() {
  ...
  const enrichmentByShell = useMemo(() => {
    return Object.fromEntries(
      shells.map((row) => {
        ...
        return [row.shellKey, suggestions];
      }),
    );
  ...
  const suggestionCount = Object.values(enrichmentByShell).reduce(
    (sum, suggestions) => sum + suggestions.length,
    0,
  );

  const approvedSuggestionCount = Object.values(suggestionDecisions).filter(
    (state) => state === "approved",
  ).length;

  const rejectedSuggestionCount = Object.values(suggestionDecisions).filter(
    (state) => state === "rejected",
  ).length;

  const avgConfidence =
    shells.length > 0
      ? Math.round(shells.reduce((sum, row) => sum + row.confidence, 0) / shells.length)
      : 0;
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="max-w-3xl text-[13px] text-[var(--wk-text-muted)]">
            Registry-owned provisional releases created when downstream surfaces cannot confidently
            match an incoming track or release. Charts can create demand for shells, but the registry
            owns review, enrichment, deduplication, and canonicalization.
          </p>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[12px] text-[var(--wk-text-muted)]">
              This page replaces the chart-owned release-shell mental model. Chart ingestion remains
              a source surface, while registry pages own the lifecycle. Phase 9 now adds an enrichment
              suggestion review layer before canonical writes.
            </p>
          </div>
        </div>
      </WkSurface>
          ...
          <table className="w-full text-left text-[13px]">
            ...
            <tbody>
              {filtered.map((row) => {
                ...
                const suggestions = enrichmentByShell[row.shellKey] ?? [];
                const pendingSuggestions = suggestions.filter(
                  (suggestion) => suggestion.status === "draft" || suggestion.status === "needs_review",
                ).length;
                ...
                return (
                  <Fragment key={row.shellKey}>
                    <tr className="border-b border-[var(--wk-border)]/60 hover:bg-[var(--wk-surface-raised)]/60">
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => navigate(`/admin/charts/ingest-runs/${row.sourceRunId}`)}
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex rounded-full bg-[var(--wk-brand-soft)] px-2 py-0.5 text-[11px] font-semibold text-[var(--wk-brand)]">
                          {pendingSuggestions}/{suggestions.length} pending
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {state === "approved" || state === "rejected" ? (
                            ...
                            <>
                              <button
                                onClick={() => markState(row, "reviewing")}
                                className="rounded px-2 py-1 text-[11px] font-semibold text-[var(--wk-text-muted)] hover:bg-[var(--wk-surface-raised)]"
                              >
                                Review
                              </button>
                              <button
                                onClick={() => markState(row, "approved")}
                                className="rounded px-2 py-1 text-[11px] font-semibold text-[var(--wk-brand)] hover:bg-[var(--wk-brand-soft)]"
                              >
                                Approve shell
                              </button>
                              <button
                                onClick={() => markState(row, "rejected")}
                                className="rounded px-2 py-1 text-[11px] font-semibold text-[var(--wk-danger)] hover:bg-[var(--wk-danger-soft)]"
                              >
                                Reject
                              </button>
                            </>
                          ...
                          <div className="grid gap-3 lg:grid-cols-[1.2fr_1fr]">
                                    ...
                                    <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                                      <div>
                                        ...
                                        <p className="text-[11px] text-[var(--wk-text-faint)]">
                                          Current: {suggestion.currentValue ?? "empty"} · {suggestion.provider} ·{" "}
                                          {suggestion.confidence}% · Status: {suggestion.status}
                                        </p>
                                      </div>
                            ...
                            <div className="rounded-xl border border-[var(--wk-border)] bg-[var(--wk-surface)] p-4">
                              ...
                              <div className="mt-3 space-y-2 text-[12px] text-[var(--wk-text-muted)]">
                                ...
                                <p>
                                  <span className="font-semibold text-[var(--wk-text)]">
                                    Provider entity links:
                                  </span>{" "}
                                  1 source link pending
                                </p>
                                <p>
                                  <span className="font-semibold text-[var(--wk-text)]">
                                    Canonical writes:
                                  </span>{" "}
                                  disabled until controlled runner approval
                                </p>
                              </div>
                            </div>
                          </div>
        ...
        {filtered.length === 0 && (
          <div className="px-4 py-14 text-center">
            ...
            <p className="text-[14px] font-bold text-[var(--wk-text)]">
              No registry release shells
            </p>
            <p className="mt-1 text-[12px] text-[var(--wk-text-muted)]">
              Release shells will appear here when downstream surfaces request provisional registry entities.
            </p>
          </div>
        )}
      </WkSurface>
