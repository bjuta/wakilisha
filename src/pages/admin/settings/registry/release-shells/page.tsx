    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        <button onClick={load} className="wk-button wk-button-ghost wk-button-sm whitespace-nowrap">
          <WkIcon name="RefreshCcw" size={14} /> Refresh
        </button>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[12px] text-[var(--wk-text-muted)]">
              ...
              Phase 9 will attach provider observations, enrichment suggestions, and provider entity links here before any canonical writes.
            </p>
          </div>
        </div>
      </WkSurface>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => navigate("/admin/settings/registry")} className="wk-button wk-button-ghost wk-button-sm whitespace-nowrap">
            Registry Settings
          </button>
          <button onClick={() => navigate("/admin/charts/release-shells")} className="wk-button wk-button-ghost wk-button-sm whitespace-nowrap">
            Legacy chart view
          </button>
        </div>
      </div>
          ...
          <table className="w-full text-left text-[13px]">
            <thead>
              <tr className="border-b border-[var(--wk-border)]">
                {["#", "Release / Track", "Artist", "Registry shell", "Source surface", "Confidence", "Review"].map((heading) => (
                  <th key={heading} className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[var(--wk-text-muted)]">{heading}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {state === "approved" || state === "rejected" ? (
                          <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${state === "approved" ? "bg-[var(--wk-success-soft)] text-[var(--wk-success)]" : "bg-[var(--wk-danger-soft)] text-[var(--wk-danger)]"}`}>
                            {state}
                          </span>
                        ) : (
                          <>
      ...
      <WkSurface className="p-4">
        <p className="text-[12px] font-bold uppercase tracking-wide text-[var(--wk-text-muted)]">Phase 9 enrichment readiness</p>
        <div className="mt-3 grid gap-2 md:grid-cols-3">
          {["Provider observations", "Enrichment suggestions", "Provider entity links"].map((label) => (
            <div key={label} className="rounded-lg border border-[var(--wk-border)] bg-[var(--wk-bg)] p-3">
              <p className="text-[13px] font-bold text-[var(--wk-text)]">{label}</p>
              <p className="mt-1 text-[11px] text-[var(--wk-text-muted)]">Phase 9 will surface this registry enrichment context for each shell.</p>
            </div>
          ))}
        </div>
      </WkSurface>
