import type { CuratorProfileData } from "../sectionTypes";

export default function CuratorProfileSection({ data }: { data: CuratorProfileData }) {
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          <div className="lg:w-[45%] flex-shrink-0">
            <div className="rounded-2xl overflow-hidden border border-[var(--wk-border)]">
              <img src={data.image} alt={data.title} className="w-full h-auto object-cover" loading="lazy" />
            </div>
          </div>
          ...
          <div className="lg:w-[55%] flex flex-col justify-center">
            ...
            <p className="text-[15px] md:text-[16px] leading-relaxed text-[var(--wk-text-soft)] mb-8">{data.bio}</p>

            <div className="space-y-4" aria-label="Career milestones">
              {data.timeline.map((item, index) => (
                <div key={item.year} className="flex items-start gap-4">
                  <div className="flex flex-col items-center">
                    ...
                    {index < data.timeline.length - 1 && <div className="w-px h-6 bg-[var(--wk-border)] mt-1" />}
                  </div>
                  <div className="pt-1">
                    <time className="text-[12px] font-bold text-[var(--wk-text)]">{item.year}</time>
                    <p className="text-[13px] text-[var(--wk-text-muted)] mt-0.5">{item.event}</p>
                  </div>
                </div>
              ))}
            </div>
