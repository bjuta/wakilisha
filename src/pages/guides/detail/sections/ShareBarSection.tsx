...
import type { ShareBarData } from "../sectionTypes";
...
export default function ShareBarSection({ data }: { data: ShareBarData }) {
  const [showMore, setShowMore] = useState(false);
  const [copied, setCopied] = useState(false);

  const handleShare = useCallback((networkId: string) => {
    const url = shareUrl(networkId, data.url, data.title, data.description);
    ...
    <section className="w-full py-6 border-b border-[var(--wk-divider)]" style={{ background: "var(--wk-bg)" }}>
      <div className="wk-container-wide px-6 flex flex-wrap items-center gap-3">
        ...
        <span className="flex flex-wrap items-center gap-2">
          {NETWORKS.map((n) => (
            <button key={n.id} type="button" onClick={() => handleShare(n.id)} className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-[var(--wk-text-muted)] hover:text-[var(--wk-text)] transition-colors rounded-md hover:bg-[var(--wk-surface-raised)]" aria-label={`Share on ${n.label}`} title={`Share on ${n.label}`}>
      ...
      {showMore && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-label="More ways to share" onClick={() => setShowMore(false)}>
          <div className="w-full max-w-md rounded-xl p-6" style={{ background: "var(--wk-surface)" }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-base font-bold text-[var(--wk-text)]">More ways to share</h3>
                <p className="text-xs text-[var(--wk-text-muted)] mt-0.5">{data.title}</p>
              </div>
              <button type="button" onClick={() => setShowMore(false)} className="w-8 h-8 flex items-center justify-center rounded-lg text-[var(--wk-text-muted)] hover:text-[var(--wk-text)] hover:bg-[var(--wk-surface-raised)] transition-colors" aria-label="Close">
                <i className="ri-close-line text-lg" />
              </button>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {MORE_NETWORKS.map((n) => (
                <button key={n.id} type="button" onClick={() => { handleShare(n.id); setShowMore(false); }} className="flex flex-col items-center gap-1.5 p-3 rounded-lg text-xs text-[var(--wk-text-muted)] hover:text-[var(--wk-text)] hover:bg-[var(--wk-surface-raised)] transition-colors" aria-label={`Share on ${n.label}`}>
                  <i className={`${n.icon} text-lg`} />
                  <span>{n.label}</span>
                </button>
              ))}
              <button type="button" onClick={() => { handleCopy(); setShowMore(false); }} className="flex flex-col items-center gap-1.5 p-3 rounded-lg text-xs text-[var(--wk-text-muted)] hover:text-[var(--wk-text)] hover:bg-[var(--wk-surface-raised)] transition-colors" aria-label="Copy link">
