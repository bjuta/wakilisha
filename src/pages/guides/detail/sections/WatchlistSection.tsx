import type { WatchlistData } from "../sectionTypes";

export default function WatchlistSection({ data }: { data: WatchlistData }) {
    ...
    <section id="signals" className="w-full py-16 md:py-24 border-t border-[var(--wk-divider)]" style={{ background: "var(--wk-bg-alt, var(--wk-bg))" }}>
      <div className="wk-container-wide px-6 md:px-10 lg:px-16">
        ...
        <ol className="space-y-8">
          {data.items.map((item) => (
            <li key={item.number} className="flex flex-col md:flex-row md:items-start gap-4 md:gap-6 pb-8 border-b border-[var(--wk-divider)] last:border-0 last:pb-0">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[11px] font-bold uppercase tracking-wider text-[var(--wk-v-fashion)] mb-1">{item.signal}</div>
                <h3 className="text-xl md:text-2xl font-bold text-[var(--wk-text)] mb-2">{item.question}</h3>
