...
export default function Search() {
  ...
  useEffect(() => {
    const nextUrlQuery = searchParams.get("q") ?? "";
    setQuery((current) => current === nextUrlQuery ? current : nextUrlQuery);
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer);
  }, [q, query, total, results.artists.length, results.tracks.length, results.releases.length, results.genres.length, results.labels.length, results.charts.length]);
  ...
  const handlePlayTrack = (track: TrackSearchItem) => {
    const playable = {
      ...
      mediaKind: "music_track" as const,
      playbackAvailability: track.previewUrl ? "excerpt" as const : "unavailable" as const,
    };
    playTrack(
      playable,
      [playable],
      { pageType: "search", entityType: "track", entitySlug: track.slug, sourceSection: "search_results", label: "Search" },
    );
  };
  ...
  const handleResultClick = (entityType: string, entitySlug: string, position: number) => {
    trackEvent("search_result_click", {
      ...
      entityType,
      entitySlug,
      context: { search_query: query.trim(), click_position: position, active_tab: activeTab },
    });
  };
  ...
  const handleTabSwitch = (tab: Tab) => {
    if (prevTabRef.current !== tab) {
      trackEvent("search_tab_switch", {
        pageType: "search",
        context: { from_tab: prevTabRef.current, to_tab: tab, search_query: query.trim(), total_results: total },
      });
    ...
    <div className="min-h-screen">
      <div className="wk-container-wide px-6 py-10 md:py-14">
        ...
        <div className="relative max-w-2xl">
          ...
          <input type="text" value={query} onChange={(event) => updateQuery(event.target.value)} placeholder="Search artists, songs, releases, genres, labels, charts..." className="w-full rounded-full border border-[var(--wk-border)] bg-[var(--wk-surface)] py-3.5 pl-12 pr-4 text-[15px] text-[var(--wk-text)] placeholder:text-[var(--wk-text-faint)] outline-none focus:border-[var(--wk-brand)]" autoFocus />
          {query && <button onClick={() => updateQuery("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--wk-text-faint)] hover:text-[var(--wk-text)]"><i className="ri-close-line text-lg" /></button>}
        </div>
      </div>

      {q && <div className="sticky top-0 z-10 border-b border-[var(--wk-border)]" style={{ background: "var(--wk-bg)" }}><div className="wk-container-wide flex gap-1 overflow-x-auto px-6 py-2 scrollbar-hide">{TABS.map((tab) => { const count = tab === "All" ? total : tab === "Artists" ? results.artists.length : tab === "Tracks" ? results.tracks.length : tab === "Releases" ? results.releases.length : tab === "Genres" ? results.genres.length : tab === "Labels" ? results.labels.length : results.charts.length; return <button key={tab} onClick={() => handleTabSwitch(tab)} className={`flex-none rounded-full px-4 py-2 text-[13px] font-semibold transition-all whitespace-nowrap ${activeTab === tab ? "bg-[var(--wk-brand)] text-[var(--wk-brand-on)]" : "border border-[var(--wk-border)] text-[var(--wk-text-soft)] hover:bg-[var(--wk-surface-raised)]"}`}>{tab}{count > 0 && <span className="ml-1.5 text-[11px] opacity-80">{count}</span>}</button>; })}</div></div>}

      <div className="wk-container-wide px-6 py-8 md:py-12">
        {!q && <div className="space-y-12"><div><div className="wk-eyebrow mb-4">Browse</div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{[
        ...
        ].map((cat) => <Link key={cat.to} to={cat.to} className="group flex flex-col gap-2 rounded-xl border border-[var(--wk-border)] bg-[var(--wk-surface)] p-4 transition-all hover:border-[var(--wk-border-2)] hover:bg-[var(--wk-surface-raised)]"><div className="flex items-center gap-2"><i className={`${cat.icon} text-[var(--wk-brand)]`} /><span className="text-[14px] font-bold text-[var(--wk-text)]">{cat.label}</span></div><span className="text-[12px] text-[var(--wk-text-muted)]">{cat.desc}</span></Link>)}</div></div><div><div className="wk-eyebrow mb-4">Jump in</div>{trendingLoading ? <div className="flex flex-wrap gap-2">{Array.from({ length: 8 }).map((_, i) => <span key={i} className="rounded-full border border-[var(--wk-border)] bg-[var(--wk-surface)] px-4 py-2"><SkeletonBlock className="h-4 w-20 rounded" /></span>)}</div> : trendingArtists.length === 0 ? <p className="text-[13px] text-[var(--wk-text-muted)]">Type a name, sound, place, or issue thread to get started.</p> : <div className="flex flex-wrap gap-2">{trendingArtists.map((name) => <button key={name} onClick={() => updateQuery(name)} className="rounded-full border border-[var(--wk-border)] bg-[var(--wk-surface)] px-4 py-2 text-[13px] font-semibold text-[var(--wk-text-soft)] transition-all hover:border-[var(--wk-brand)]/40 hover:text-[var(--wk-brand)]">{name}</button>)}</div>}</div></div>}

        {q && loading && <div className="py-20 space-y-6"><div className="max-w-2xl mx-auto space-y-3">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="animate-pulse flex items-center gap-4 rounded-xl border border-[var(--wk-border)] bg-[var(--wk-surface)] p-4"><SkeletonBlock className="h-12 w-12 rounded-md" /><div className="flex-1 space-y-2"><SkeletonBlock className="h-4 w-1/3 rounded" /><SkeletonBlock className="h-3 w-2/3 rounded" /></div></div>)}</div></div>}
        {q && !loading && total === 0 && <div className="py-20 text-center text-[var(--wk-text-muted)]"><i className="ri-search-line mb-4 block text-5xl" /><div className="text-[18px] font-bold text-[var(--wk-text)] mb-2">Nothing came up for "{query}"</div><div className="text-[14px]">Try another spelling, another name, or browse a different section below.</div></div>}

        {q && !loading && <div className="space-y-10">
          ...
          {showTracks && results.tracks.length > 0 && <section><SectionHeader title="Tracks" count={results.tracks.length} onViewAll={activeTab === "All" && results.tracks.length > 6 ? () => setActiveTab("Tracks") : undefined} /><div className="overflow-hidden rounded-2xl border border-[var(--wk-border)] bg-[var(--wk-surface)]"><div className="divide-y divide-[var(--wk-divider)]">{results.tracks.slice(0, activeTab === "All" ? 6 : undefined).map((track, idx) => { const meta = [track.artist, track.genre, track.label].filter(Boolean).join(" · "); return <div key={track.id} className="group flex items-center gap-3 px-5 py-3 transition-colors hover:bg-[var(--wk-surface-raised)]"><PlayableArtwork label={track.title} onPlay={(event) => { event.stopPropagation(); handlePlayTrack(track); }} disabled={!track.isPlayable} className="h-12 w-12 rounded-md bg-[var(--wk-surface-raised)]"><img src={track.artworkUrl} alt="" className="h-full w-full object-cover" /></PlayableArtwork><div className="min-w-0 flex-1"><Link to={`/tracks/${slugify(track.artist)}/${track.slug}`} onClick={() => handleResultClick("track", track.slug, idx + 1)} className="text-[13px] font-bold text-[var(--wk-text)] hover:underline">{highlight(track.title, query)}</Link>{meta && <div className="text-[11px] text-[var(--wk-text-muted)]">{highlight(meta, query)}</div>}{track.contextText && <p className="mt-1 line-clamp-2 text-[12px] leading-snug text-[var(--wk-text-soft)]">{highlight(track.contextText, query)}</p>}</div><TrackActionsMenu registryTrackId={track.id} trackTitle={track.title} artistName={track.artist} artistSlug={track.artistSlug} artworkUrl={track.artworkUrl} trackSlug={track.slug} /></div>; })}</div></div></section>}
          ...
          {showCharts && results.charts.length > 0 && <section><SectionHeader title="Chart entries" count={results.charts.length} /><div className="overflow-hidden rounded-2xl border border-[var(--wk-border)] bg-[var(--wk-surface)]"><div className="divide-y divide-[var(--wk-divider)]">{results.charts.map((entry, idx) => <div key={`${entry.rank}-${entry.slug}`} className="flex items-center gap-3 px-5 py-3"><div className="w-6 text-right text-[14px] font-black text-[var(--wk-brand)]">{entry.rank}</div><div className="h-10 w-10 shrink-0 overflow-hidden rounded-md bg-[var(--wk-surface-raised)]"><img src={entry.artworkUrl} alt="" className="h-full w-full object-cover" /></div><div className="min-w-0 flex-1"><Link to={`/tracks/${slugify(entry.artist)}/${entry.slug}`} onClick={() => handleResultClick("chart_entry", entry.slug, idx + 1)} className="text-[13px] font-bold text-[var(--wk-text)] hover:underline">{highlight(entry.title, query)}</Link><div className="text-[11px] text-[var(--wk-text-muted)]">{highlight(entry.artist, query)}</div>{entry.contextText && <p className="mt-1 line-clamp-2 text-[12px] leading-snug text-[var(--wk-text-soft)]">{highlight(entry.contextText, query)}</p>}</div><TrackActionsMenu registryTrackId={entry.canonicalTrackId} trackTitle={entry.title} artistName={entry.artist} artistSlug={slugify(entry.artist)} artworkUrl={entry.artworkUrl} trackSlug={entry.slug} /></div>)}</div></div></section>}
        </div>}
      </div>
    </div>
  );
