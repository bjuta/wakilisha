          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_420px] items-end">
            ...
            <div>
              ...
              <p className="mt-4 max-w-lg text-[16px] leading-relaxed" style={{ color: "var(--wk-text-soft)" }}>
                {seriesData.description}. The definitive ranking of African music, rebuilt from the repaired cultural
                registry.
              </p>
      ...
      <div className="border-b border-[var(--wk-border)]" style={{ background: "var(--wk-surface)" }}>
        <div className="wk-container-wide flex gap-3 overflow-x-auto px-6 py-4 scrollbar-hide">
          {CHART_SERIES.map((s) => (
            ...
            >
              <div
                className={`text-[13px] font-bold ${activeSeries === s.id ? "text-[var(--wk-brand)]" : "text-[var(--wk-text)]"}`}
              >
                {s.label}
              </div>
      ...
      {!loading && (
        <section className="wk-container-wide px-6 py-12 md:py-20">
          <div className="mb-8">
            ...
            <h3 className="font-black text-[clamp(24px,3vw,36px)] leading-[1.02] tracking-[-0.038em] text-[var(--wk-text)]">
              Chart milestones
            </h3>
          </div>
