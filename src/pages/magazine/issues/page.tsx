...
export default function AllIssuesPage() {
  ...
  const allIssues = useMemo<IssueWithExperience[]>(() => {
    return buildMagazineIssues(articles).map((issue) => ({
      issue,
  ...
  const firstExperience = allIssues[0]?.experience;

  const filteredIssues = useMemo(() => {
    if (!search.trim()) return visibleIssues;
    const q = search.toLowerCase().trim();
    return visibleIssues.filter(({ issue, experience }) => {
      const haystack = [
        issue.title,
        issue.issueLabel,
    ...
    <main className="magazine-issue min-h-screen">
      <section className="max-w-[1180px] mx-auto px-5 md:px-8 pt-12 md:pt-20 pb-10">
        ...
        <h1 className="font-[Fraunces] text-[clamp(48px,9vw,128px)] font-light tracking-[-.04em] leading-[.84] text-[var(--wk-text)] mt-4">
          Choose the <em className="text-[var(--wk-brand)] italic">door in.</em>
        </h1>
        <p className="mt-5 max-w-[62ch] text-[15px] leading-relaxed text-[var(--wk-text-muted)]">
          Every issue is built around a different cultural shape: a sound, a room, a memory, an argument, an image, a system, or a route worth following.
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-6">
          ...
          >
            {showBackfilled ? "Show latest doors" : "Show every issue"}
          ...
          <div className="relative flex-1 max-w-[440px]">
            ...
            <input
              ...
              placeholder="Search by sound, scene, memory, system..."
        ...
        {filteredIssues.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-[15px] text-[var(--wk-text-muted)]">
              {search ? firstExperience?.emptyState ?? "No issue matches that thread yet." : "No issues have opened yet."}
            </p>
            {search && (
              <button
                onClick={() => setSearch("")}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredIssues.map(({ issue, experience }, index) => (
              <IssueCard
                key={issue.slug}
                issue={issue}
    ...
    <section className="max-w-[1180px] mx-auto px-5 md:px-8 pb-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--wk-text-faint)]">
              Artists to follow through the issues
            </span>
            <span className="flex-1 h-px bg-[var(--wk-border)]" />
            <Link to="/artists" className="text-[11px] font-bold text-[var(--wk-brand)] hover:text-[var(--wk-brand-2)] transition-colors flex items-center gap-1 whitespace-nowrap">
              All artists <i className="ri-arrow-right-line text-[11px]" />
            </Link>
          </div>
          <div className="flex flex-wrap gap-2">
            {artists.map((artist) => (
              <Link key={artist.slug} to={`/artists/${artist.slug}`} className="inline-flex items-center gap-2.5 rounded-full border border-[var(--wk-border)] bg-[var(--wk-surface)] px-4 py-2 hover:border-[var(--wk-brand)] hover:bg-[var(--wk-brand-soft)] transition-all group">
                ...
                <span className="text-[12px] font-semibold text-[var(--wk-text)] group-hover:text-[var(--wk-brand)] transition-colors whitespace-nowrap">{artist.title}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      {releases.length > 0 && (
        <div>
          <div className="flex items-center gap-3 mb-4">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--wk-text-faint)]">
              Recent releases near the magazine
            </span>
            <span className="flex-1 h-px bg-[var(--wk-border)]" />
            <Link to="/releases" className="text-[11px] font-bold text-[var(--wk-brand)] hover:text-[var(--wk-brand-2)] transition-colors flex items-center gap-1 whitespace-nowrap">
              All releases <i className="ri-arrow-right-line text-[11px]" />
            </Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {releases.map((release) => (
              <Link key={release.slug} to={releaseUrl({ slug: release.slug, artist: release.author })} className="group flex items-center gap-3 rounded-xl border border-[var(--wk-border)] bg-[var(--wk-surface)] p-3 hover:border-[var(--wk-brand)] transition-all">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-bold text-[var(--wk-text)] group-hover:text-[var(--wk-brand)] transition-colors truncate">{release.title}</p>
                  <p className="text-[10px] text-[var(--wk-text-muted)]">{release.author}{release.releaseType && ` · ${release.releaseType}`}</p>
                </div>
              </Link>
            ))}
          </div>
...
function IssueCard({
  ...
  const mood = experience.issueMood;
  ...
  return (
    <Link
      to={issueUrl(issue)}
      className="group relative overflow-hidden rounded-xl min-h-[420px] flex flex-col transition-all duration-500 mag-meaning-card"
      style={{
    ...
    >
      <div className="absolute inset-0 opacity-30 transition-opacity duration-500 group-hover:opacity-45" style={{ background: `radial-gradient(70% 50% at 50% 30%, ${palette.accent}18, transparent 70%)` }} />

      <div className="relative z-10 flex-1 flex flex-col p-7 md:p-9">
        <div className="flex items-center justify-between gap-3">
          <span className="mag-archetype-badge" style={{ color: palette.accent }}>
            {experience.archetypeLabel}
          </span>
          {isNewest && (
            <span className="text-[10px] font-black uppercase tracking-[.14em] px-2.5 py-1 rounded-full" style={{ background: `${palette.accent}22`, color: palette.accent }}>
              Latest
            </span>
          )}
        </div>

        <div className="mt-6 mb-4 text-[11px] font-black uppercase tracking-[.16em]" style={{ color: palette.accent }}>
          {experience.issueCta}
        </div>

        <h2 className="font-[Fraunces] text-[clamp(36px,5vw,56px)] font-light tracking-[-.035em] leading-[.9] mt-auto" style={{ color: palette.text }}>
          {issue.title}
        </h2>

        <p className="mt-3 text-[14px] leading-relaxed max-w-[48ch]" style={{ color: isLight ? "rgba(26,21,12,.58)" : "rgba(255,255,255,.62)" }}>
          {experience.archiveBlurb}
        </p>

        <p className="mag-card-why mt-4" style={{ color: isLight ? "rgba(26,21,12,.52)" : "rgba(255,255,255,.52)" }}>
          {experience.cardBlurb}
        </p>

        <div className="flex items-center gap-3 mt-5 flex-wrap">
          <span className="text-[11px] font-semibold" style={{ color: palette.accent }}>
            {issue.articles.length} stories
          </span>
          <span className="text-[11px]" style={{ color: isLight ? "rgba(26,21,12,.4)" : "rgba(255,255,255,.35)" }}>
            {experience.searchSnippet}
          </span>
        </div>

        {previewArticle && (
          <div className="mt-4 pt-4 border-t transition-all duration-400 overflow-hidden" style={{ borderColor: isLight ? "rgba(26,21,12,.1)" : "rgba(255,255,255,.1)", maxHeight: isHovered ? "130px" : "0px", opacity: isHovered ? 1 : 0, marginTop: isHovered ? "16px" : "0", paddingTop: isHovered ? "16px" : "0" }}>
            <p className="text-[10px] font-black uppercase tracking-[.14em] mb-1" style={{ color: palette.accent }}>Start here</p>
            <p className="text-[13px] font-semibold leading-snug" style={{ color: palette.text }}>{previewArticle.title}</p>
            <p className="text-[11px] mt-0.5" style={{ color: isLight ? "rgba(26,21,12,.45)" : "rgba(255,255,255,.45)" }}>{previewArticle.author} · {previewArticle.readingTime} min</p>
          </div>
        )}
      </div>

      <div className="absolute bottom-0 left-0 right-0 h-[3px] transition-all duration-400" style={{ background: palette.accent, opacity: isHovered ? 1 : 0 }} />
    </Link>
  );
}
