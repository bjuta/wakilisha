      ...
      <div className="mag-cover-inner">
        ...
        <div className="mag-cover-foot">
          ...
          <div className="mag-cover-coords magazine-meta">
            {issue.sourceWindowLabel}<br />{editorial.coverVariant.replace(/-/g, " ")}<br /><b>WAKILISHA.AFRICA</b>
          </div>
        </div>
      </div>
    </section>
        ...
        <div>
          ...
          {note.mode === "playlist-note" && note.playlist?.length ? (
            <div className="mag-editor-playlist">
              {note.playlist.map((item, idx) => <Link key={item.slug} to={`/magazine/${item.slug}`}><span>{String(idx + 1).padStart(2, "0")}</span>{item.title}</Link>)}
            </div>
          ) : null}
          {note.mode === "song-note" && note.lovedRelease ? (
            <Link className="mag-editor-loved" to={`/magazine/${note.lovedRelease.slug}`}><Bolt /> <span>{note.lovedRelease.title}</span></Link>
          ) : null}
          <div className="mag-note-flow">
            {note.body.map((paragraph, idx) => <p key={idx}>{paragraph}</p>)}
    ...
    <section className="magazine-spread paper">
      <div className="mag-toc">
        <div className="mag-toc-top">
          <Masthead small />
          <div className="magazine-meta" style={{ textAlign: "right", color: "var(--mag-ink-muted)", lineHeight: 1.9 }}>Contents<br />{issue.issueLabel}<br />{issue.sourceWindowLabel}</div>
        </div>
        <h2 className="mag-toc-title">{splitEmphasis(editorial.contentsTitle)}</h2>
        <div className="mag-toc-lead">
          <div className="mag-toc-hero">Start here: <b>{hero?.title ?? issue.title}</b> — the piece that gives this issue its first pulse.</div>
          ...
          {sectionGroups.map((section, idx) => (
            <div className="mag-toc-block" key={section.id}>
              ...
              {(section.articles ?? []).slice(0, 4).map((article, articleIdx) => (
                <Link className="mag-toc-line" key={article.slug} to={`/magazine/${article.slug}`}><span>{articleIdx === 0 ? <b>{article.title}</b> : article.title}</span><span className="pg">{14 + idx * 8 + articleIdx}</span></Link>
              ))}
            </div>
          ))}
        </div>
...
function FeatureSpread({ spread, editorial }: { spread: MagazineSpread; editorial: MagazineEditorialSystem }) {
  ...
  return (
    <section className={`magazine-spread dark mag-feature-${editorial.featureVisualMode}`}>
      <div className="mag-feature-open">
        <FeatureVisual article={article} editorial={editorial} />
        <div className="mag-feature-content">
          <div className="mag-rail magazine-meta"><span>{editorial.featureFrame.eyebrow}</span><Masthead small /></div>
          {editorial.featureFrame.routeLabel && <div className="mag-route-label magazine-meta">{editorial.featureFrame.routeLabel}</div>}
          <h2 className="mag-feature-title">{article.title}</h2>
          <p className="mag-feature-deck">{article.dek}</p>
        </div>
      </div>
      <div className="mag-feature-body">
        <div className="mag-copy">{sampleParagraphs(article).map((paragraph, idx) => <p key={idx}>{paragraph}</p>)}</div>
        <aside className="mag-side-card">
          <h4>{editorial.featureFrame.titlePrefix ?? "Field note"}</h4>
          <p style={{ color: "var(--mag-on-dark-muted)", fontSize: 12 }}>{editorial.featureFrame.fieldNote}</p>
          <h4 style={{ marginTop: 22 }}>Related record</h4>
          {(spread.articles ?? []).slice(0, 4).map((item) => <div className="mag-side-item" key={item.slug}><Link to={`/magazine/${item.slug}`}>{item.title}</Link><p>{item.canonicalSection} · {item.readingTime} min</p></div>)}
        </aside>
      </div>
    </section>
  );
...
function FeatureVisual({ article, editorial }: { article: MagazineIssueArticle; editorial: MagazineEditorialSystem }) {
  if (editorial.featureVisualMode === "issue-one-route") {
