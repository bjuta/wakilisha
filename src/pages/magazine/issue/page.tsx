...
function Bolt({ className = "" }: { className?: string }) {
  return <span className={`mag-bolt ${className}`} aria-hidden="true"><svg viewBox="121.5 0 20.5 30" focusable="false"><path fill="currentColor" d="M132.91,11.14l-7.87,18.73,15.96-17.97c.26-.29.05-.76-.34-.76h-7.75Z" /><path fill="currentColor" d="M130.72.18h6.59c.15.01.26.17.2.31-2.24,5.23-4.48,10.46-6.73,15.69l-6.74-.02c-.19,0-.32-.19-.24-.37l6.54-15.37c.06-.15.21-.25.37-.25Z" /></svg></span>;
}
...
function MagazineSeal({ size = "medium" }: { size?: "small" | "medium" | "cover" }) {
  ...
  return <span className={`mag-seal ${size}`} aria-label="WAKILISHA field-record seal"><svg viewBox="0 0 100 100" role="img"><defs><path id={`seal-ring-${pathId}`} d="M50,50 m-36,0 a36,36 0 1,1 72,0 a36,36 0 1,1 -72,0" /></defs><circle cx="50" cy="50" r="46.5" fill="none" stroke="currentColor" strokeWidth="1" /><circle cx="50" cy="50" r="40" fill="none" stroke="currentColor" strokeOpacity=".14" strokeWidth=".5" /><text className="ring-text" fill="currentColor"><textPath href={`#seal-ring-${pathId}`} startOffset="0%">· RECORDED IN NAIROBI · WAKILISHA FIELD RECORD · </textPath></text><g transform="translate(50,52) scale(1.5) translate(-132.4,-15)"><path fill="currentColor" d="M132.91,11.14l-7.87,18.73,15.96-17.97c.26-.29.05-.76-.34-.76h-7.75Z" /><path fill="currentColor" d="M130.72.18h6.59c.15.01.26.17.2.31-2.24,5.23-4.48,10.46-6.73,15.69l-6.74-.02c-.19,0-.32-.19-.24-.37l6.54-15.37c.06-.15.21-.25.37-.25Z" /></g></svg></span>;
}
...
function IssueCover({ issue, editorial }: { issue: MagazineIssue; editorial: MagazineEditorialSystem }) {
  ...
  return <section className={`magazine-spread dark mag-cover mag-cover-${editorial.coverVariant} mag-mood-${editorial.issueMood}`}>{coverImage && <img className="mag-cover-trace" src={coverImage} alt="" />}<div className="mag-cover-inner"><div className="mag-cover-masthead"><Masthead /><div className="mag-cover-row magazine-meta"><span>Magazine</span><b>{issue.issueLabel} · {issue.sourceEndDate.getFullYear()}</b><span>Nairobi, Kenya</span></div></div><div className="mag-cover-center">{editorial.coverVariant !== "type-cover" && <MagazineSeal size="cover" />}<div className="mag-cover-eyebrow magazine-meta">{issue.subtitle}</div><h1 className="mag-cover-title">{coverTitle(issue.title)}</h1><p className="mag-cover-deck">{issue.deck}</p></div><div className="mag-cover-foot"><div className="mag-cover-bills">{bills.map((article, idx) => <Link key={article.slug} to={`/magazine/${article.slug}`} className="mag-cover-bill"><span className="n magazine-meta">P.{String(14 + idx * 7).padStart(2, "0")}</span><Bolt /><span>{article.title}</span></Link>)}</div><div className="mag-cover-coords magazine-meta">{issue.sourceWindowLabel}<br />{editorial.coverVariant.replace(/-/g, " ")}<br /><b>WAKILISHA.AFRICA</b></div></div></div></section>;
}
...
function EditorsNoteSpread({ issue, editorial }: { issue: MagazineIssue; editorial: MagazineEditorialSystem }) {
  ...
  return <section className={`magazine-spread paper mag-note-${note.mode}`}><div className="mag-note"><aside className="mag-note-side"><div className="mag-note-label magazine-meta">{note.eyebrow}</div><p className="mag-editor-name">{editorial.editor.name}</p><p className="mag-editor-role">{editorial.editor.role}</p><p>{issue.issueLabel} · {issue.sourceWindowLabel}</p><p>{issue.primaryVerticals.slice(0, 3).join(" / ")}</p>{note.mode !== "image-note" && <MagazineSeal size="small" />}</aside><div>{note.imageUrl && <figure className="mag-editor-image"><img src={note.imageUrl} alt="" /><figcaption>{note.imageCaption}</figcaption></figure>}<p className="mag-note-open">{note.title}</p>{note.mode === "playlist-note" && note.playlist?.length ? <div className="mag-editor-playlist">{note.playlist.map((item, idx) => <Link key={item.slug} to={`/magazine/${item.slug}`}><span>{String(idx + 1).padStart(2, "0")}</span>{item.title}</Link>)}</div> : null}{note.mode === "song-note" && note.lovedRelease ? <Link className="mag-editor-loved" to={`/magazine/${note.lovedRelease.slug}`}><Bolt /> <span>{note.lovedRelease.title}</span></Link> : null}<div className="mag-note-flow">{note.body.map((paragraph, idx) => <p key={idx}>{paragraph}</p>)}{note.pull && <p className="mag-pull">{note.pull}</p>}</div></div></div></section>;
}
...
function sampleParagraphs(article: MagazineIssueArticle) {
  ...
  return [article.dek || "This story anchors a larger cultural signal inside the issue.", "The magazine engine treats this feature as the issue’s strongest available entry point.", "What matters is the movement: scenes, records, language, places, rights, and the people carrying them into memory."];
}

function FullBleedImageSpread({ spread }: { spread: MagazineSpread }) {
  const article = spread.articles?.[0];
  if (!article?.heroUrl) return null;
  return <section className={`magazine-spread mag-fullbleed mag-fullbleed-${spread.variant ?? "title-overlay"}`} style={{ ["--section-accent" as string]: spread.accent ?? "var(--mag-green)" }}><img src={article.heroUrl} alt="" /><div className="mag-fullbleed-shade" /><div className="mag-fullbleed-copy"><div className="magazine-meta">{spread.eyebrow ?? "Image record"}</div><h2>{article.title}</h2><p>{article.dek}</p></div></section>;
}

function QuoteOnlySpread({ spread }: { spread: MagazineSpread }) {
  const article = spread.articles?.[0];
  return <section className={`magazine-spread mag-quote-only mag-quote-${spread.variant ?? "accent"}`} style={{ ["--section-accent" as string]: spread.accent ?? "var(--mag-green)" }}><div className="mag-quote-inner"><div className="magazine-meta">{spread.eyebrow ?? "Quote"}</div><blockquote>“{spread.title}”</blockquote>{article && <Link to={`/magazine/${article.slug}`}>From {article.title}</Link>}</div></section>;
}

function ColorInterludeSpread({ spread }: { spread: MagazineSpread }) {
  return <section className="magazine-spread mag-color-interlude" style={{ ["--section-accent" as string]: spread.accent ?? "var(--mag-green)" }}><div className="mag-color-interlude-inner"><div className="magazine-meta">{spread.eyebrow ?? "Interlude"}</div><h2>{splitEmphasis(spread.title)}</h2><p>{spread.deck}</p>{spread.articles?.[0] && <Link to={`/magazine/${spread.articles[0].slug}`}>Open the first record →</Link>}</div></section>;
}
...
function RenderSpread({ spread, issue, editorial, index }: { spread: MagazineSpread; issue: MagazineIssue; editorial: MagazineEditorialSystem; index: number }) {
  switch (spread.type) {
    case "cover": return <IssueCover issue={issue} editorial={editorial} />;
    case "editors-note": return <EditorsNoteSpread issue={issue} editorial={editorial} />;
    case "contents": return <ContentsSpread issue={issue} editorial={editorial} />;
    case "feature": return <FeatureSpread spread={spread} editorial={editorial} />;
    case "signal": return <SignalSpread spread={spread} editorial={editorial} />;
    case "full-bleed-image": return <FullBleedImageSpread spread={spread} />;
    case "quote-only": return <QuoteOnlySpread spread={spread} />;
    case "color-interlude": return <ColorInterludeSpread spread={spread} />;
    ...
    case "back-matter": return <BackMatterSpread issue={issue} editorial={editorial} />;
