...
const migration =
  "docs/engineering/replay-baseline/legacy-migrations/20260714185539_phase_0a_lock_down_user_access_admin_rpcs.sql";
...
for (const signature of targets) {
  ...
  const revoke =
    `revoke execute on function ${target} from public, anon, authenticated`;
  const grant =
    `grant execute on function ${target} to service_role`;

  if (!compact.includes(revoke)) {
    throw new Error(
      `Missing public/anon/authenticated revoke for ${signature}`,
    );
  }

  if (!compact.includes(grant)) {
    throw new Error(
...
console.log(
  `Verified ${targets.length} privileged admin RPC execution boundaries.`,
);
