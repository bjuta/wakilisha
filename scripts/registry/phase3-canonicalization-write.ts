...
type ArtistMatch = { id: string; slug: string; name: string };
...
function getArtistText(row: DecisionRow): string {
  ...
  return textField(resolution, ["canonicalPrimaryArtistName", "primaryArtistName", "artistText", "artistName", "primaryArtist"])
    || textField(after, ["canonicalPrimaryArtistName", "primaryArtistName", "artistText", "artistName", "primaryArtist"])
...
function getArtistSlug(row: DecisionRow): string {
  ...
  return textField(resolution, ["canonicalPrimaryArtistSlug", "primaryArtistSlug", "artistSlug"])
    || textField(after, ["canonicalPrimaryArtistSlug", "primaryArtistSlug", "artistSlug"])
...
async function relationshipAlreadyExists(pool: ReturnType<typeof createRegistryPool>, row: PlanRow): Promise<boolean> {
  ...
  if (row.intended_table === "registry_release_artists") {
    const exists = await pool.query(
      `select 1 from public.registry_release_artists where release_id = $1 and artist_id = $2 and role = $3 and credit_order = 1 limit 1`,
      [row.entity_id, row.artist_id, row.role],
    );
  ...
  if (row.intended_table === "registry_track_artists") {
    const exists = await pool.query(
      `select 1 from public.registry_track_artists where track_id = $1 and artist_id = $2 and role = $3 and credit_order = 1 limit 1`,
      [row.entity_id, row.artist_id, row.role],
    );
    return Boolean(exists.rowCount);
  }
...
async function buildPlan(pool: ReturnType<typeof createRegistryPool>, decisions: DecisionRow[]): Promise<PlanRow[]> {
  ...
  for (const decision of decisions) {
    ...
    if (decision.decision_type === "approve_featured_artist_split") {
      plan.push({ ...base, status: "blocked", reason: "featured_artist_split_requires_phase3c" });
      continue;
    }

    if (decision.decision_type !== "approve_primary_artist") {
      plan.push({ ...base, status: "blocked", reason: "decision_type_not_supported_by_phase3b" });
      continue;
    }
...
async function insertCanonicalRow(pool: ReturnType<typeof createRegistryPool>, row: PlanRow): Promise<"inserted" | "already_exists"> {
  if (!row.entity_id || !row.artist_id || !row.intended_table || !row.role) {
    throw new Error(`Cannot insert incomplete plan row for decision ${row.decision_id}`);
  }

  const metadata = {
    phase: "phase3b_canonicalization_runner",
    decisionId: row.decision_id,
    reviewItemId: row.review_item_id,
    reviewType: row.review_type,
    source: "phase2b_structured_resolution",
  };
  ...
  if (row.intended_table === "registry_release_artists") {
    const result = await pool.query(
      ...
      values ($1, $2, $3, $4, $5, true, false, 1, $4, 'phase3b_canonicalization_runner', 100, 'canonical', $6::jsonb, now())
      on conflict (release_id, artist_id, role, credit_order)
      where release_id is not null and artist_id is not null
      do nothing
      returning id
      `,
      [row.entity_id, row.artist_id, row.artist_slug, row.artist_name, row.role, JSON.stringify(metadata)],
    );
    return result.rowCount ? "inserted" : "already_exists";
  }
  ...
  if (row.intended_table === "registry_track_artists") {
    const result = await pool.query(
      ...
      values ($1, $2, $3, $4, $5, true, false, 1, $4, 'phase3b_canonicalization_runner', 100, 'canonical', $6::jsonb, now())
      on conflict (track_id, artist_id, role, credit_order)
  ...
  throw new Error(`Unsupported intended table: ${row.intended_table}`);
}

async function writeActionableRows(pool: ReturnType<typeof createRegistryPool>, plan: PlanRow[]): Promise<Array<Record<string, unknown>>> {
  ...
  try {
    for (const row of actionable) {
      const writeResult = await insertCanonicalRow(pool, row);
      results.push({
      ...
      if (writeResult === "inserted") {
        await pool.query(
          `
          insert into public.registry_audit_log (actor_label, action, entity_type, entity_id, before_value, after_value, metadata)
          values (
            'system',
            'phase3b_canonical_artist_relationship_written',
            $1,
            $2,
            '{}'::jsonb,
            $3::jsonb,
          ...
          [
            ...
            row.entity_id,
            JSON.stringify({
              intendedTable: row.intended_table,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log("\nWAKILISHA Phase 3B Canonicalization Write Runner");
    console.log("=".repeat(80));
    console.log(writeMode ? "Mode: WRITE. Actionable canonical relationships will be inserted idempotently." : "Mode: DRY RUN. No canonical tables will be modified.");
    ...
    printSection("Actionable canonical write plan");
    ...
    if (dryRunMode) {
      ...
      console.table([{
        ...
        actionable_writes_planned: actionable.length,
        canonical_tables_modified: false,
        write_mode_used: false,
      }]);
      console.log("\nDry run complete. To write actionable rows, rerun with: npm run registry:phase3:write -- --write --limit=100");
      return;
    }

    const writeResults = await writeActionableRows(pool, plan);

    printSection("Write results");
    ...
    console.table([{
      decisions_scanned: decisions.length,
      actionable_writes_planned: actionable.length,
      rows_inserted: writeResults.filter((row) => row.result === "inserted").length,
      rows_already_existing: writeResults.filter((row) => row.result === "already_exists").length,
      canonical_tables_modified: writeResults.some((row) => row.result === "inserted"),
      public_rendering_changed: false,
      public_api_changed: false,
    }]);

    console.log("\nPhase 3B write complete. Only actionable primary artist relationships were inserted.");
  } finally {
    await pool.end();
  }
}
...
main().catch((error) => {
  console.error("[phase3-canonicalization-write] Failed:", error instanceof Error ? error.message : error);
  process.exitCode = 1;
});
