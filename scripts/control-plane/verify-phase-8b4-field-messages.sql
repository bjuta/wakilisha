...
begin
  if not exists(select 1 from supabase_migrations.schema_migrations where version='20260907075239' and name='phase_8b4_field_messages')
  ...
  if position('allow_human_reply' in v_send_definition)=0 or position('field_submission_conversation_scope' in v_send_definition)=0 then
    raise exception 'PHASE_8B4_FIELD_FAIL: send_message lost Candidate A or Candidate B guard';
  end if;
  if position('can_start' in v_access_definition)=0 or position('has_field_scope' in v_access_definition)=0 then
    raise exception 'PHASE_8B4_FIELD_FAIL: Messages access projection lacks scoped-start split';
  end if;
  if position('follow_up_permission' in v_scope_definition)=0 or position('preferred_contact_channel' in v_scope_definition)=0 or position('count(*)' in v_scope_definition)=0 then
    raise exception 'PHASE_8B4_FIELD_FAIL: Field Conversation scope is not current-policy constrained';
  end if;
  if position('view_field_intake' in v_bridge_definition)=0 or position('view_restricted_field_sources' in v_bridge_definition)=0 or position('expected_submission_revision' in v_bridge_definition)=0 then
...
begin
  create temporary table phase8b4_field_fixture(
    editor_id uuid, contributor_id uuid, author_id uuid, retired_id uuid,
    editor_person uuid, contributor_person uuid, author_person uuid, retired_person uuid,
    field_resource_id uuid, alternate_field_resource_id uuid, retired_field_resource_id uuid,
    conversation_id uuid, message_id uuid, receipt_id uuid
  ) on commit drop;
...
union all select contributor_id,'field_contributor','active',null::uuid,now(),'Phase 8B.4 rollback verifier',now(),now() from phase8b4_field_fixture
union all select author_id,'author','active',null::uuid,now(),'Phase 8B.4 rollback verifier',now(),now() from phase8b4_field_fixture
union all select retired_id,'field_contributor','active',null::uuid,now(),'Phase 8B.4 rollback verifier',now(),now() from phase8b4_field_fixture;
...
union all select alternate_field_resource_id,'field_submission',contributor_id,'private','active',contributor_id from phase8b4_field_fixture
union all select retired_field_resource_id,'field_submission',retired_id,'private','active',retired_id from phase8b4_field_fixture;
...
union all select alternate_field_resource_id,'field_submission','FS-20260907-A1B2C3D4E6',contributor_id,'authenticated',1,'received','standard','do_not_name','account_contact','allowed','email','owns_or_controls','granted','none','internal','none','not_collected',contributor_id,contributor_id,now(),gen_random_uuid() from phase8b4_field_fixture
union all select retired_field_resource_id,'field_submission','FS-20260907-A1B2C3D4E7',retired_id,'authenticated',1,'received','standard','do_not_name','account_contact','allowed','messages','owns_or_controls','granted','none','internal','none','not_collected',retired_id,retired_id,now(),gen_random_uuid() from phase8b4_field_fixture;

update editorial.person_identity_links
set link_state='retired',retired_at=now(),retired_reason='Rollback verifier identity retirement'
...
do $assertions$
declare
  v_replay boolean; v_message_count integer; v_ref_count integer; v_notification_count integer;
  v_access jsonb; v_editor_detail jsonb; v_contributor_detail jsonb; v_author_detail jsonb;
  v_search integer; v_receipt_principal text;
begin
