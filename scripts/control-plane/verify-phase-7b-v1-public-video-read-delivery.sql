...
begin
  ...
  if position('current_published_version_id' in v_public_def) = 0
     or position('video.assert_publishable_publication_version' in v_public_def) = 0
     or position('variant_role = ''video_transcode''' in v_public_def) = 0
     or position('https://media.wakilisha.africa/derivatives/' in v_public_def) = 0
  then
    raise exception 'PHASE_7B_V1_FAIL: public Video reader lacks immutable/public derivative guards';
  end if;

  if position('/video/captions/' in v_public_def) = 0
     or position('private-files/captions/' in v_public_def) = 0
  then
    raise exception 'PHASE_7B_V1_FAIL: public Video reader lacks governed caption relationship';
  end if;

  if position('storage_path' in v_public_def) > 0
     and position('''storage_path''' in v_public_def) > 0
  then
