...
PRODUCTION_GA_MEASUREMENT_ID="G-ML0E5BEL57"
...
printf '\n=== PRODUCTION PUBLIC BUILD CONFIG ===\n'
export VITE_GA_MEASUREMENT_ID="$PRODUCTION_GA_MEASUREMENT_ID"

if [ "$VITE_GA_MEASUREMENT_ID" != "G-ML0E5BEL57" ]; then
  echo 'GATE_D_PRODUCTION_FINISH=FAIL_GA4_MEASUREMENT_ID_AUTHORITY'
  exit 1
fi

printf 'VITE_GA_MEASUREMENT_ID=%s\n' "$VITE_GA_MEASUREMENT_ID"
echo 'PRODUCTION_PUBLIC_BUILD_CONFIG=PASS'

printf '\n=== EXACT MERGED-MAIN DEPENDENCIES ===\n'
npm ci --prefer-offline --no-audit --no-fund

test -x node_modules/.bin/vitest || {
  echo 'GATE_D_PRODUCTION_FINISH=FAIL_VITEST_DEPENDENCY_MISSING'
  exit 1
}

test -f node_modules/typescript/package.json || {
  echo 'GATE_D_PRODUCTION_FINISH=FAIL_TYPESCRIPT_DEPENDENCY_MISSING'
  exit 1
}

echo 'MERGED_MAIN_DEPENDENCIES=PASS'
